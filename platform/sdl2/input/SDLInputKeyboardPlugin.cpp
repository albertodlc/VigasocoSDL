...
void SDLInputKeyboardPlugin::acquire() { SDL_SetRelativeMouseMode(SDL_TRUE); }
...
void SDLInputKeyboardPlugin::process(int *inputs) {
  ...
  if (joy) {
    ...
    keystate[SDLK_UP] |=
        (hat & SDL_HAT_UP) && !(hat & SDL_HAT_DOWN) && !keystate[SDLK_DOWN];

    keystate[SDLK_DOWN] |=
        (hat & SDL_HAT_DOWN) && !(hat & SDL_HAT_UP) && !keystate[SDLK_UP];

    keystate[SDLK_RIGHT] |=
        (hat & SDL_HAT_RIGHT) && !(hat & SDL_HAT_LEFT) && !keystate[SDLK_LEFT];

    keystate[SDLK_LEFT] |=
        (hat & SDL_HAT_LEFT) && !(hat & SDL_HAT_RIGHT) && !keystate[SDLK_RIGHT];
#endif // _EE

#ifdef _PS3
    keystate[SDLK_UP] |=
        SDL_JoystickGetButton(joy, PAD_UP) && !keystate[SDLK_DOWN];
    keystate[SDLK_DOWN] |=
        SDL_JoystickGetButton(joy, PAD_DOWN) && !keystate[SDLK_UP];
    keystate[SDLK_RIGHT] |=
        (SDL_JoystickGetButton(joy, PAD_RIGHT) && !keystate[SDLK_LEFT]);
    keystate[SDLK_LEFT] |=
        (SDL_JoystickGetButton(joy, PAD_LEFT) && !keystate[SDLK_RIGHT]);
...
void SDLInputKeyboardPlugin::initRemapTable() {
  ...
  g_keyMapping[KEYBOARD_INTRO] = SDL_SCANCODE_RETURN;
...
const std::string SDLInputKeyboardPlugin::g_properties[] = {"keyConfig"};

const unsigned int SDLInputKeyboardPlugin::g_paramTypes[] = {PARAM_ARRAY |
                                                             PARAM_INPUT};
...
void SDLInputKeyboardPlugin::setProperty(std::string prop, int index,
                                         int data) {
  if (prop == "keyConfig") {
...
int SDLInputKeyboardPlugin::getProperty(std::string prop) const { return -1; };
