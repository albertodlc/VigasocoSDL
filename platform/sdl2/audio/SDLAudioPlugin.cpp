...
void SDLAudioPlugin::Play(int sample, bool loop) {
  ...
  try {
    ...
    std::cerr << "Play " << sample << " " << loop << " " << o.what()
              << std::endl;
  }
  /*
  catch (std::range_error e)
  {
          std::cerr << "Play " << sample << " " << loop << " " << e.what() <<
  std::endl;
  }*/
}
...
bool SDLAudioPlugin::LoadWAV(const UINT8 *data, const UINT32 len) {
  ...
  if (SDL_LoadWAV_RW(SDL_RWFromMem((UINT8 *)data, len), 0, &wave, &audio_buf,
                     &dlen) == NULL) {
...
bool SDLAudioPlugin::LoadWAV_internal(const SDL_AudioSpec &wave,
                                      Uint8 *audio_buf, const Uint32 dlen) {
  SDL_AudioCVT cvt;
  sound s;
  ...
  if (SDL_BuildAudioCVT(&cvt, wave.format, wave.channels, wave.freq,
                        fmt_real.format, fmt_real.channels,
                        fmt_real.freq) == -1) {
...
void SDLAudioPlugin::mix(UINT8 *stream, int len) {
  ...
  for (tIteratorSounds it = sounds.begin(); it != sounds.end(); it++) {
    if (it->active) {
      ...
      if (amount > len)
        amount = len;
      SDL_MixAudio(stream, &it->data[it->dpos], amount,
                   mute ? 0 : SDL_MIX_MAXVOLUME);
      it->dpos += amount;
      if (it->dpos == it->dlen) {
        if (!it->loop)
          it->active = false;
...
const unsigned int SDLAudioPlugin::g_paramTypes[] = {PARAM_ARRAY | PARAM_INPUT};
...
int SDLAudioPlugin::getProperty(std::string prop) const { return -1; };
