...
bool SDLBasicDrawPlugin<T>::init(const VideoInfo *vi, IPalette *pal) {
  ...
  if (myPixels == NULL) {
    SDL_ShowSimpleMessageBox(
        SDL_MESSAGEBOX_ERROR, "Memoria VigasocoSDLDrawPlugin ERROR",
        "No se puede reservar memoria para los gráficos", NULL);
    return false;
  }

  SDL_CreateWindowAndRenderer(vi->width, vi->height, _flags, &window,
                              &renderer);

  if (window == NULL || renderer == NULL) {
    fprintf(stderr, "Couldn't set %dx%dx%d video mode: %s\n", vi->width,
            vi->height, _bpp, SDL_GetError());
    return false;
  }
  ...
  if (SDL_RenderSetLogicalSize(renderer, vi->width, vi->height) < 0) {
    SDL_ShowSimpleMessageBox(
        ...
        "SDL_RenderSetLogicalSize VigasocoSDLDrawPlugin ERROR", SDL_GetError(),
        NULL);
    return false;
  }
...
template <typename T> void SDLBasicDrawPlugin<T>::end() {
  if (_originalPalette)
...
// drawing methods
template <typename T> void SDLBasicDrawPlugin<T>::render(bool throttle) {
#ifdef _EE
