...
GameDriver::GameDriver(std::string driverName, std::string fullName,
                       int intsPerSecond) {
...
bool GameDriver::init(IPalette *pal) {
  ...
  _videoInfo.refreshRate =
      _numInterruptsPerSecond / _numInterruptsPerVideoUpdate;
  ...
  for (i = 0; i < _gameFiles.size(); i++) {
    ...
    if (!theFileLoader->loadGameData(_driverName, _gameFiles[i])) {
      ...
      for (int f = 0; f < _gameFiles[i]->getNumFiles(); f++) {
        ...
        buffer << std::endl
               << file->fileName << " (size = 0x" << file->fileSize
...
bool GameDriver::decodeGraphics() {
  ...
  while (i < _gfxEncoding.size()) {
    ...
    if (_gameFiles[j]->getType() == GRAPHICS) {
      ...
      _gfx.push_back(
          GfxDecoder::decode(_gfxEncoding[i], _gameFiles[j]->getData()));
