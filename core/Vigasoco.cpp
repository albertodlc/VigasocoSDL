...
bool Vigasoco::init(std::string name) {
  ...
  // inits the palette (the last 2 colors are used by the core to display
  // information)
  ...
  */
  // TODO: VGA6, mientras probamos con VGA, metemos esto a fuego en el color 253
  // y 254
  ...
  if (!_timingHandler->init(_timer, _driver->getNumInterruptsPerSecond(),
                            _driver->getNumInterruptsPerVideoUpdate(),
                            _driver->getnumInterruptsPerLogicUpdate())) {
 ...
 *
 */
void Vigasoco::initFrame() { _numFrames++; }
...
void Vigasoco::processCoreInputs() {
  ...
  if (_inputHandler->hasBeenPressed(FUNCTION_8)) {
    ...
    if (_actualVideoFrameSkip < 0)
      _actualVideoFrameSkip = TimingHandler::FRAMESKIP_LEVELS - 1;
  ...
  if (_inputHandler->hasBeenPressed(FUNCTION_9)) {
    _actualVideoFrameSkip = _timingHandler->getVideoFrameSkip() + 1;
    if (_actualVideoFrameSkip > (TimingHandler::FRAMESKIP_LEVELS - 1))
      _actualVideoFrameSkip = 0;
    _timingHandler->setVideoFrameSkip(_actualVideoFrameSkip);
  }
...
void Vigasoco::showFPS(bool skipThisFrame, bool isEnabled) {
  ...
  if ((_numFrames % (TimingHandler::FRAMES_PER_FPS_UPDATE) * (frameSkip + 1)) ==
      0) {
