...
void CPC6128::setHardwareColor(IPalette *pal, int numInk, int color) {
  ...
  pal->setColor(numInk, CPC6128::hardwarePalette[color][0],
                CPC6128::hardwarePalette[color][1],
                CPC6128::hardwarePalette[color][2]);
}
...
void CPC6128::setInkColor(IPalette *pal, int numInk, int color) {
  ...
  pal->setColor(numInk, CPC6128::hardwarePalette[inkColors[color]][0],
                CPC6128::hardwarePalette[inkColors[color]][1],
                CPC6128::hardwarePalette[inkColors[color]][2]);
}
...
// sets a pixel in mode 0 (160x200, x pixels = 4 width, y pixels = 2 height, 16
// colors)
...
// sets a pixel in mode 1 (320x200, x pixels = 2 width, y pixels = 2 height, 4
// colors)
...
// sets a pixel in mode 1 (320x200, x pixels = 2 width, y pixels = 2 height, 256
// colors)
...
// sets a pixel in mode 2 (640x200, x pixels = 1 width, y pixels = 2 height, 2
// colors)
...
// gets a pixel in mode 0 (160x200, x pixels = 4 width, y pixels = 2 height, 16
// colors)
...
// gets a pixel in mode 1 (320x200, x pixels = 2 width, y pixels = 2 height, 4
// colors)
...
// gets a pixel in mode 2 (640x200, x pixels = 1 width, y pixels = 2 height, 2
// colors)
...
// fills a rectange in mode 0 (160x200, x pixels = 4 width, y pixels = 2 height,
// 16 colors)
...
void CPC6128::fillMode1Rect(int x, int y, int width, int height, int color) {
  ...
  // CPC assert((color >= 0) && (color < 4));
  assert((color >= 0) &&
         (color < 256)); // VGA ya que se llama en Marcador::dibujaBarra y ya se
                         // usan colores VGA
