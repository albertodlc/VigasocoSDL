...
public:
  ...
  // al igual que en CPC
  // UINT8 DirtyPixels[sizeof(screenBuffer)/8]; // pixeles modificados en la
  // CPC6128 video RAM
  UINT8 DirtyPixels[(640 * 200) /
                    8]; // pixeles modificados en la CPC6128 video RAM
...
public:
  ...
  inline int packPixelMode0(int oldByte, int pixel, int color) {
    ...
    static int byteColors[16] = {0x00, 0xc0, 0x44, 0xcc, 0x30, 0xf0,
                                 0x74, 0xfc, 0x03, 0xc3, 0x47, 0xcf,
                                 0x33, 0xf3, 0x77, 0xff};
