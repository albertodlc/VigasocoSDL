...
#include "stdio.h"
*/
bool TimingHandler::init(ITimer *t, int intsPerSecond, int intsPerVideoUpdate,
                         int intsPerLogicUpdate) {
  _timer = t;
  ...
  _timePerInterrupt =
      (double)_timer->getTicksPerSecond() / (double)intsPerSecond;

  /* DEBUG
  printf("%ld %ld %g %g %g %g\n",startTime,endTime,_ticksPerSleepMiliSec,
                  _ticksPerSleepMiliSec2,_ticksPerMiliSecond,_timePerInterrupt);
  //std::cout << startTime << " " << endTime << " " << _ticksPerSleepMiliSec <<
  " " <<
  //		_ticksPerSleepMiliSec2 << " " << _ticksPerMiliSecond << " " <<
  _timePerInterrupt;
  */
  ...
  _lastFrameBase = _timer->getTime() -
                   (int)(INTERRUPTS_PER_BASE_TIME_UPDATE *
                         _numInterruptsPerVideoUpdate * _timePerInterrupt);
...
void TimingHandler::waitThisInterrupt() {
  ...
  if (_throttle) {
    _numIntsModeBaseTimeUpdateInts =
        _interruptNum %
        (INTERRUPTS_PER_BASE_TIME_UPDATE * _numInterruptsPerVideoUpdate);
    speedThrottle();
  }

  // if we have to update video in this interrupt, update frameskip count and
...
void TimingHandler::endThisInterrupt() { _interruptNum++; }

void TimingHandler::speedThrottle() {
  // recalculate base values each
  // INTERRUPTS_PER_BASE_TIME_UPDATE*_numInterruptsPerVideoUpdate frames
  if (_numIntsModeBaseTimeUpdateInts == 0) {
    _thisFrameBase = _lastFrameBase +
                     (int)(INTERRUPTS_PER_BASE_TIME_UPDATE *
                           _numInterruptsPerVideoUpdate * _timePerInterrupt);
  }

  INT64 targetTime = _thisFrameBase +
                     (int)(_numIntsModeBaseTimeUpdateInts * _timePerInterrupt);
  ...
  if (currentTime - targetTime < 0) {
    ...
    while (currentTime - targetTime < 0) {
      if ((targetTime - currentTime) > _ticksPerSleepMiliSec * 1.20) {
        ...
        _ticksPerSleepMiliSec = _ticksPerSleepMiliSec * 0.90 +
                                ((double)(nextTime - currentTime)) * 0.10;
...
void TimingHandler::sleep(UINT32 milliSeconds) {
  ...
  milliSeconds =
      (UINT32)(milliSeconds * (((double)(FRAMESKIP_LEVELS - _videoFrameSkip)) /
                               FRAMESKIP_LEVELS));
  ...
  if (currentTime - targetTime < 0) {
    ...
    while (currentTime - targetTime < 0) {
      if ((targetTime - currentTime) > _ticksPerSleepMiliSec2 * 1.10) {
        ...
        // evolutive adjust sleep time
        _ticksPerSleepMiliSec2 = _ticksPerSleepMiliSec2 * 0.85 +
                                 ((double)(nextTime - currentTime)) * 0.15;
        currentTime = nextTime;
      } else {
        currentTime = _timer->getTime();
      }
...
void TimingHandler::computeFPS() {
  ...
  if ((!_lastVideoFrameSkipped) &&
      (_perfomance.framesSinceLastFPS >=
       FRAMES_PER_FPS_UPDATE * (_videoFrameSkip + 1))) {
    INT64 currentTime = _timer->getTime();
    double secsElapsed = (double)(currentTime - _perfomance.lastFpsTime) *
                         (1.0 / _timer->getTicksPerSecond());

    // set perfomance data
    _perfomance.currentFPS =
        (double)_perfomance.framesSinceLastFPS / secsElapsed;
