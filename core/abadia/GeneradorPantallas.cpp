...
*/
  ...
  for (int i = 0; i < sizeof(parches) / sizeof(struct parche) &&
    ...
    if (parches[i].pantalla == numPantalla) {
      ...
      bufferTiles[parches[i].y][parches[i].x].tile[a] = parches[i].fg;
      bufferTiles[parches[i].y][parches[i].x].tile[a - 1] = parches[i].bg;
      if (numPantalla == 0x14) {
        /*				printf("prof %d %d %d %d\nprofy %d %d %d
...
//			bit 0: si es 1 indica que el bloque puede ocultar a los
//sprites 		byte 1: 			bits 7-5: parámetro 1 (su función depende del tipo de bloque a
//construir) 			bits 4-0: posición inicial en x (sistema de coordenadas del buffer
//de tiles) 		byte 2: 			bits 7-5: parámetro 2 (su función depende del tipo de bloque
//a construir) 			bits 4-0: posición inicial en y (sistema de coordenadas del
//buffer de tiles) 		byte 3: altura inicial del bloque
//
/////////////////////////////////////////////////////////////////////////////

// genera los capas de tiles que forman una pantalla dados los datos de los
// bloques que la forman
void GeneradorPantallas::genera(UINT8 *datosPantalla) {
  // inicia la pila
  posPila = 0;

  // repite el proceso de generación de bloques hasta que no se encuentre el
  // marcador de fin de datos
  while (*datosPantalla != 0xff) {

    if (datosPantalla[0] == 0x2f) {
      if ((datosPantalla[1] == 0x0e) && (datosPantalla[2] == 0xb0) &&
          (datosPantalla[3] == 0x0e)) {
        int a = 0;
      }
    }
    // los 7 bits más significativos del primer byte de los datos indican el
    // tipo de bloque a construir
    ...
    // si la entrada es de 4 bytes, el cuarto byte indica la altura del bloque,
    // y también implica que se calcularán los datos de profundidad a lo largo
    // del proceso de generación del bloque
...
void GeneradorPantallas::iniciaInterpretacionBloque(UINT8 *tilesBloque,
                                                    bool modificaTiles,
                                                    int altura) {
  if (modificaTiles) {
  ...
  // transforma la posición del bloque en el buffer de tiles al sistema de
...
// transforma la posición del bloque en el buffer de tiles al sistema de
// coordenadas de la rejilla las ecuaciones de cambio de sistema de coordenadas
// son: mapa de tiles -> rejilla:		Xrejilla = Ymapa + Xmapa - 15
//									Yrejilla
//= Ymapa - Xmapa + 16
...
void GeneradorPantallas::actualizaTile(int tile, TileInfo *tileDesc) {
  ...
  for (int i = 1; i < nivelesProfTiles; i++) {
    ...
    // si el nuevo elemento que se dibuja en este tile tiene menor profundidad
    // que el elemento que estaba antes en este tile, ajusta la profundidad del
    // elemento anterior a la profundidad del elemento
    ...
    // pasa los datos anteriores de mayor profundidad a la capa de menor
    // profundidad
...
void GeneradorPantallas::grabaTile(int tile) {
  // comprueba si el tile es de la parte central, trasladando la parte central
  // al origen
  int posX = tilePosX - 8;
  int posY = tilePosY - 8;
...
int GeneradorPantallas::leeDatoORegistro(int *posReg) {
  ...
  // en otro caso, es una lectura de un registro del buffer con los datos del
  // bloque
  int pos = dato - 0x61;
...
// dibuja en pantalla el contenido del buffer de tiles desde el centro hacia
// fuera
void GeneradorPantallas::dibujaBufferTiles() {
  ...
  while (abajo < 20) {
    ...
    // dibuja 4 tiras: una hacia abajo, otra a la derecha, otra hacia arriba y
...
void GeneradorPantallas::dibujaTira(int &x, int &y, int deltaX, int deltaY,
                                    int veces) {
  // para cada tile de la tira
  for (int i = 0; i < veces; i++) {
    // por cada capa de profundidad
    for (int k = 0; k < nivelesProfTiles; k++) {
      // obtiene el número de tile asociado a esta profundidad del buffer de
      // tiles
        ...
        for (int j = 0; j < 8; j++){
                ...
                for (int i = 0; i < 4; i++){
                        ...
                        for (int k = 0; k < 4; k++){
                                ...
                                // combina el color del pixel de pantalla con el
nuevo color = (oldColor & mascaras[numTabla + 1][color]) |
mascaras[numTabla][color];
...
void GeneradorPantallas::dibujaTile(int x, int y, int num) {
  ...
  // VGA
  // en el remake de PC cada pixel ocupa un byte y no vienen en pack de 4, como
  // en la version CPC y cada tile ocupa 8*16 bytes parece que con 0x1400 va
  // bien (5888 del tile 6 que localizamos - 768 (6 tiles por 16*8 bytes que
  // tiene un tile) = 5120 , que es 0x1400 en hexadecimal

  UINT8 *tileData =
      &roms[0x24000 - 1 - 0x4000 + 0x1400 +
            num * 8 * 16]; // pasamos los 0x20000 de la rom CPC y ahi esta el
                           // GraficosVGA 0x20000 de los 0x24000 de rom CPC
                           // menos los 0x4000 de la presentacion que el puntero
                           // de roms se salta en el constructor de Juego.cpp
