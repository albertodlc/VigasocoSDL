...
public:
  ...
  AccionesDia
      *accionesDia; // ejecutor de las acciones dependiendo del momento del día
  ...
  int duracionMomentoDia; // indica lo que falta para pasar al siguiente momento
                          // del día
  int oldMomentoDia;      // indica el momento del día de las últimas acciones
  ...
  int numeroRomano; // indica el número romano de la habitación del espejo (en
                    // el caso de que se haya generado)
  int despDatosAlturaEspejo; // desplazamiento hasta el final de los datos de
                             // altura de la habitación del espejo
  int despBloqueEspejo; // desplazamiento hasta los datos del bloque que forma
                        // el espejo
  ...
  int cntTiempoAOscuras; // contador del tiempo que pueden ir a oscuras por la
                         // biblioteca

  int cntLeeLibroSinGuantes; // contador para llevar un control del tiempo que
                             // lee guillermo el libro sin los guantes
  bool
      pergaminoGuardado; // indica que el pergamino lo tiene el abad en su
                         // habitación o está detrás de la habitación del espejo

  int numeroAleatorio; // número aleatorio

  bool hayMovimiento; // cuando hay algún movimiento de un personaje, esto se
                      // pone a true
  int cntMovimiento;  // contador que se pone a 0 con cada movimiento de
                      // guillermo (usado para cambios de cámara)

  int numPersonajeCamara;    // indica el personaje al que sigue la cámara
                             // actualmente
  int opcionPersonajeCamara; // indica el personaje al que podría seguir la
                             // cámara si no hay movimiento
...
public:
  ...
  friend std::ifstream &operator>>(std::ifstream &in, Logica *const logica);
