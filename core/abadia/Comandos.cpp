...
void call(GeneradorPantallas *gen, bool modificaTiles) {
  ...
  gen->iniciaInterpretacionBloque(tilesBloque, modificaTiles,
                                  gen->datosBloque[14]);
...
void avanzaHastaFinDeWhile(GeneradorPantallas *gen) {
  ...
  while (profWhile > 0) {
    ...
    if (dato == 0x82) {
      ...
      // en otro caso, sigue pasando y contando los while a los que entra y a
      // los que sale
...
// inicia la ejecución de una serie de instrucciones mientras el parámetro 1 sea
// > 0
bool WhileParam1::ejecutar(GeneradorPantallas *gen) {
  // obtiene el valor del parámetro 1
  int aux = gen->obtenerRegistro(0x6d, 0);

  // si el bucle se va a ejecutar alguna vez, inserta en la pila la dirección de
  // retorno y el valor actual del parámetro 1
  if (aux > 0) {
    gen->push(gen->comandosBloque);
    gen->push(aux);
  } else {
...
// inicia la ejecución de una serie de instrucciones mientras el parámetro 2 sea
// > 0
bool WhileParam2::ejecutar(GeneradorPantallas *gen) {
  ...
  // si el bucle se va a ejecutar alguna vez, inserta en la pila la dirección de
  // retorno y el valor actual del parámetro 2
  if (aux > 0) {
    gen->push(gen->comandosBloque);
    gen->push(aux);
  } else {
...
// dibuja un tile en el buffer de tiles (si es visible), cambiando la posición
// actual en el buffer
...
bool UpdateReg::ejecutar(GeneradorPantallas *gen) {
  ...
  // si se modifica un registro de coordenadas locales de la rejilla, ajusta el
  // resultado entre 0 y 100
  if (dato >= 0x70) {
    // si no se estaban calculando las coordenadsa locales de la rejilla para
    // este bloque, sale
