...
int SpriteLuz::rellenoLuz[16] = {0x00e0, 0x03f8, 0x07fc, 0x07fc, 0x0ffe, 0x0ffe,
                                 0x1fff, 0x1fff, 0x1fff, 0x1fff, 0x0ffe, 0x0ffe,
                                 0x07fc, 0x07fc, 0x03f8, 0x00e0};
...
void SpriteLuz::ajustaAPersonaje(Personaje *pers) {
  ...
  // coloca la posición de la luz basada en la posición del personaje (ajustando
  // la posición al inicio de un tile)
...
void SpriteLuz::dibuja(Sprite *spr, UINT8 *bufferMezclas, int lgtudClipX,
  ...
  for (int j = 0; j < 15; j++) {
    ...
    for (int i = 0; i < rellenoIzquierda; i++) {
      ...
      */
      // VGA
      bufferMezclas[0] = bufferMezclas[20 * 4] = bufferMezclas[40 * 4] =
          bufferMezclas[60 * 4] = 0;
