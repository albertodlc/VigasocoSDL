...
/////////////////////////////////////////////////////////////////////////////
// tabla para el cálculo de los desplazamientos de las puertas según la
// orientación
/////////////////////////////////////////////////////////////////////////////
...
// actualiza la posición del sprite dependiendo de su posición con respecto a la
// cámara
void Puerta::notificaVisibleEnPantalla(int posXPant, int posYPant,
                                       int profundidad) {
  ...
  sprite->posXPant =
      posXPant + despOrientacion[oriEntrada][0] + despOrientacion[oriPuerta][4];
  sprite->posYPant =
      posYPant + despOrientacion[oriEntrada][1] + despOrientacion[oriPuerta][5];
...
void Puerta::compruebaAbrirCerrar(Personaje **personajes, int numPersonajes) {
  ...
  // TODO: si guillermo o adso están cerca de la puerta y no tienen permisos
  // para abrirla, se cierra
...
// comprueba si el personaje está cerca de la puerta y si es así, si se puede
// abrir
...
bool Puerta::accionesAbrirCerrar(bool abrir) {
  ...
  // pone a 0 la altura de las posiciones ocupadas por la puerta cuando estaba
  // abierta o cerrada
  marcaPosiciones(elMotorGrafico->rejilla, altura);

  int difOri = (abrir) ? 1 : -1;

  // cambia la orientación de la puerta dependiendo de hacia donde se abra o se
  // cierre
  orientacion = (haciaDentro) ? (orientacion - difOri) & 0x03
                              : (orientacion + difOri) & 0x03;
  ...
  if (rejilla->estaEnRejillaCentral(this, posXRejilla, posYRejilla)) {
    ...
    // si no hay un personaje en la posición en la que se abre o cierra la
    // puerta, sale
    // if ((rejilla->bufAlturas[posYRejilla][posXRejilla] & 0xf0) == 0) return
    // true;
...
void Puerta::marcaPosiciones(RejillaPantalla *rejilla, int valor) {
  ...
  // si la puerta está en las 20x20 posiciones centrales de la rejilla, marca
