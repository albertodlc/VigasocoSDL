...
/////////////////////////////////////////////////////////////////////////////
// tabla para el cálculo del desplazamiento, según la animación y la cámara, de
// un personaje
/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
// actualización del entorno cuando un personaje es visible en la pantalla
// actual
/////////////////////////////////////////////////////////////////////////////

void Personaje::actualizaPosPantSprite(int posXPant, int posYPant,
                                       int profundidad) {
  int entrada = 0;

  // si el personaje ocupa sólo una posición porque está en un desnivel, hay que
  // tener en cuenta varios casos
  if (enDesnivel) {
    entrada += 2;
    ...
    // si el personaje ocupa cuatro posiciones, la posición solo depende del
    // contador de la animación
    entrada += contadorAnimacion & 0x01;
  }

  // actualiza la posición en pantalla del sprite asociado al personaje
  // dependiendo de la cámara
  int oriAjustada = elMotorGrafico->ajustaOrientacionSegunCamara(orientacion);
  sprite->posXPant =
      posXPant + despX +
      difPosAnimCam[elMotorGrafico->oriCamara][oriAjustada][2 * entrada];
  sprite->posYPant =
      posYPant + despY +
      difPosAnimCam[elMotorGrafico->oriCamara][oriAjustada][2 * entrada + 1];
...
// actualiza la posición y la animación del sprite dependiendo de su posición
// con respecto a la cámara
void Personaje::notificaVisibleEnPantalla(int posXPant, int posYPant,
                                          int profundidad) {
...
Personaje::DatosFotograma *Personaje::calculaFotograma() {
  ...
  // selecciona un fotograma dependiendo de la orientación y de si el personaje
  // va hacia la derecha o a la izquierda
...
void Personaje::actualizaAnimacion(DatosFotograma *df, int profundidad) {
  ...
  // comprueba si hay que girar los gráficos del personaje por el cambio de la
...
void Personaje::avanzaAnimacion() {
  ...
  // actualiza la posición y dimensiones del sprite del personaje según la
  // animación y la cámara
  actualizaSprite();
}
...
void Personaje::actualizaSprite() {
  ...
  if (elMotorGrafico->actualizaCoordCamara(this, posXPant, posYPant,
                                           profundidad) != -1) {
...
void Personaje::mueve() {
  // pone la posición y dimensiones actuales como posición y dimensiones
  // antiguas
  sprite->preparaParaCambio();

  // si el personaje está en las posiciones centrales de la pantalla actual,
  // limpia las posiciones que ocupa
  ...
  // si el personaje está en las posiciones centrales de la pantalla actual,
  // marca las posiciones que ocupa
  marcaPosicion(elMotorGrafico->rejilla, valorPosicion);
}

// dependiendo de como esté la animación del personaje, avanza la animación o
...
void Personaje::trataDeAvanzar(int difAltura1, int difAltura2, int avanceX,
                               int avanceY) {
  bajando = false;

  // si el personaje ocupa 4 posiciones
  if (!enDesnivel) {
    // si se quiere subir o bajar, cambia la altura e indica que el personaje
    // está en desnivel
    if (difAltura1 == 1) {
      altura++;
      enDesnivel = true;
    ...
    // si se anda por una zona sin desnivel, actualiza la posición y la
    // animación del personaje según hacia donde se avanza
    if (difAltura1 == 0) {
    ...
    if ((difAltura2 == 0x10) || (difAltura2 == 0x20))
      return;
    ...
    if (!giradoEnDesnivel) {
      ...
      // si las 2 posiciones que hay avanzando tienen la misma altura, indica
      // que ya no está en desnivel
...
// si la orientación del personaje es DERECHA o ARRIBA devuelve false, en otro
// caso devuelve true
...
// indica si se puede dejar un objeto. Si no se puede, devuelve -1. En otro caso
// devuelve el número de objeto que puede dejar
int Personaje::puedeDejarObjeto(int &posXObj, int &posYObj, int &alturaObj) {
  ...
  for (int i = 0; i < Juego::numObjetos; i++) {
    ...
    if (alturaBasePlantaObj == elMotorGrafico->rejilla->minAltura) {
      ...
      // comprueba si la posición en la que se deja el objeto está en la rejilla
      // de pantalla que se muestra
      if (rejilla->ajustaAPosRejilla(posXObj, posYObj, posXObjRejilla,
                                     posYObjRejilla)) {
        int altPos = rejilla->bufAlturas[posYObjRejilla][posXObjRejilla];

        // si hay algún personaje en la posición en la que se quiere dejar el
        // objeto, sale
        if ((altPos & 0xf0) != 0)
          return -1;
        ...
        // si hay mucha diferencia de altura del personaje a donde se va dejar,
        // sale
        if ((altPos - alturaRelativa) >= 5)
          return -1;

        // si la altura de la posición en donde se va a dejar el objeto es
        // distinta de la de sus vecinos, sale
        if (altPos != rejilla->bufAlturas[posYObjRejilla][posXObjRejilla - 1])
          return -1;
        if (altPos != rejilla->bufAlturas[posYObjRejilla - 1][posXObjRejilla])
          return -1;
        if (altPos !=
            rejilla->bufAlturas[posYObjRejilla - 1][posXObjRejilla - 1])
          return -1;
...
void Personaje::marcaPosicion(RejillaPantalla *rejilla, int valor) {
  ...
  // si el personaje está en las 20x20 posiciones centrales de la rejilla, marca
  ...
  if (rejilla->estaEnRejillaCentral(this, posXRejilla, posYRejilla)) {
    ...
    rejilla->bufAlturas[posYRejilla][posXRejilla] =
        (rejilla->bufAlturas[posYRejilla][posXRejilla] & 0x0f) | valor;

    // si el personaje no está en un desnivel, ocupa 4 posiciones ((x, y)(x-1,
    // y)(x, y-1)(x-1, y-1))
    if (!enDesnivel) {
      ...
      rejilla->bufAlturas[posYRejilla - 1][posXRejilla - 1] =
          (rejilla->bufAlturas[posYRejilla - 1][posXRejilla - 1] & 0x0f) |
          valor;
    }
  }
}
