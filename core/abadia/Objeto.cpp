...
// cámara
void Objeto::notificaVisibleEnPantalla(int posXPant, int posYPant,
                                       int profundidad) {
...
bool Objeto::seHaCogidoPor(Personaje *pers, int mascara) {
  ...
  // si el objeto está cogido, su posición viene dada por la del personaje que
  // lo tiene
  if (seHaCogido) {
    // si el personaje no puede quitar objetos, sale
    if (!pers->puedeQuitarObjetos)
      return false;
  ...
  int posXPers =
      pers->posX + 2 * elMotorGrafico->tablaDespOri[pers->orientacion][0];
  if (posXObj != posXPers)
    return false;

  int posYPers =
      pers->posY + 2 * elMotorGrafico->tablaDespOri[pers->orientacion][1];
  if (posYObj != posYPers)
    return false;
  ...
  // guarda el personaje que tiene el objeto, indica que el objeto se ha cogido
  // e inicia el contador
  personaje = pers;
  seHaCogido = true;
  seEstaCogiendo = true;
...
void Objeto::dejar(Personaje *pers, int mascara, int posXObj, int posYObj,
                   int alturaObj) {
  ...
  // inicia el contador para coger/dejar objetos y le quita el objeto al
  // personaje
  pers->contadorObjetos = 0x10;
  pers->objetos = pers->objetos & (~mascara);

  // salta a la rutina de redibujado de objetos para redibujar solo el objeto
  // que se deja actualiza la posición del sprite según la cámara
  int posXPant, posYPant, sprPosY;

  if (elMotorGrafico->actualizaCoordCamara(this, posXPant, posYPant, sprPosY) !=
      -1) {
