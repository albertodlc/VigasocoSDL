...
public:
  ...
  volatile int contadorInterrupcion; // contador incrementado en la interrupción
                                     // para sincronizar el juego
...
protected:
  ...
  void flipeaGraficos(UINT8 *tablaFlip, UINT8 *src, UINT8 *dest, int ancho,
                      int bytes);
