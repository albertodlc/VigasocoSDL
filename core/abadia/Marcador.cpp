...
void Marcador::muestraDiaYMomentoDia() {
  ...
  */
  ...
  // hace que avance el momento del día, para mostrar el efecto de scroll en las
  // letras del día
...
void Marcador::avanzaMomentoDia() {
  ...
  laLogica->duracionMomentoDia =
      duracionEtapasDia[laLogica->dia - 1][laLogica->momentoDia] * 0x100;
}
...
void Marcador::dibujaDigitoDia(int digito, int x, int y) {
  ...
  cpc6128->unpackPixelMode1(*data, k));
          ...
          // si no había que mostrar ningún dígito, mantiene el puntero en los
  pixels negros if (digito == 0){ data = data - 2;
          }
  }
  */
  // VGA
  ...
  // obtiene un puntero a los gráficos del dígito
  UINT8 *data = &roms[despDigito + 0x24000 - 1 -
                      0x4000]; // pasar a la zona VGA y dentro buscar el digito
  ...
  for (int j = 0; j < 8; j++) {
    ...
    /* ?? no hace falta, en los graficos VGA estan los 8x8 pixeles negros ???
                    // si no había que mostrar ningún dígito, mantiene el
       puntero en los pixels negros if (digito == 0){ data = data - 2;
                    }
                    */
  }
}

// realiza el efecto de scroll en la parte del marcador que muestra el momento
// del día
void Marcador::realizaScrollMomentoDia() {
  ...
  if (numPosScrollDia != 0) {
    ...
    // 8 líneas de alto
    for (int j = 0; j < 8; j++) {
      // desplaza 48/8 = 6 caracteres a la izquierda 1 caracter (cada caracter
      // es de 8x8)
      for (int i = 0; i < 48; i++) {
        // CPC cpc6128->setMode1Pixel(44 + i - 8, 180 + j,
        // cpc6128->getMode1Pixel(44 + i, 180 + j)); VGA
        cpc6128->setVGAPixel(44 + i - 8, 180 + j,
                             cpc6128->getMode1Pixel(44 + i, 180 + j));
      }
    }
...
void Marcador::decrementaObsequium(int unidades) {
  ...
  if (laLogica->obsequium < 0) {
    // si guillermo no ha muerto, cambia el estado del abad para que le eche de
    // la abadía
  ...
  // CPC dibujaBarra(31 - laLogica->obsequium + 1, 3, 240 + laLogica->obsequium,
  // 177);
  dibujaBarra(31 - laLogica->obsequium + 1, 0, 240 + laLogica->obsequium,
              177); // VGA
}
...
void Marcador::dibujaMarcador() {
  // TODO igual hay que coger el marcardor del GraficosVGA de la nueva versión
  // del remake
  //  Ver si tiene 32 pixeles o 34 o 35, para dejar hueco para los 2 pixeles de
  //  mas de los acentos
  /* CPC
  // apunta a los datos gráficos del marcador
  UINT8 *data = &roms[0x1e328];

  // dibuja las 32 líneas que forman el marcador en la parte inferior de la
  pantalla for (int j = 0; j < 32; j++){ for (int i = 0; i < 256/4; i++){ for
  (int k = 0; k < 4; k++){ cpc6128->setMode1Pixel(32 + 4*i + k, 160 + j,
  cpc6128->unpackPixelMode1(*data, k));
                  }
                  data++;
          }
  ...
  */
  ...
  // la solucion alternativa adoptada en el remake PC con traducciones es
  // dibujar un nuevo marcador con espacio suficiente
      ...
      if ((objetos & (1 << (Juego::numObjetos - 1))) != 0) {
        ...
        for (int j = 0; j < spr->alto; j++) {
          ...
          for (int i = 0; i < spr->ancho * 4; i++) {
            /* CPC
            for (int k = 0; k < 4; k++){
                    cpc6128->setMode1Pixel(posX + 4*i + k, posY + j,
            cpc6128->unpackPixelMode1(*data, k));
            }
            data++;
            */
            ...
            int pixel = *data++;
            if (pixel != 255)
              cpc6128->setVGAPixel(posX + i, posY + j, pixel);
          }
        }
      } else {
...
void Marcador::imprimeFrase(std::string frase, int x, int y, int colorTexto,
                            int colorFondo) {
  /* Código original
  ...
  */
  ...
  /* Código en la rama abadIA
     donde compilamos con -std=c++11 y podemos hacer esto
          ...
          // la frase
          // Aquí convierto a utf32 y ya puedo sacar cada int con el valor de
     cada
  ...
  */
  ...
  // en la rama master no queremos reducir el número de sistemas a los que poder
  // compilar VigasocoSDL limitando a los que tienen soporte de c++11 y que
  // queremos llegar a cualquiera con un compilador de C++ básico y SDL Así que
  // ...
  ...
  while (*_frase != '\0') {
    ...
    std::string::difference_type offset =
        1; // TODO: esta var se puede ahorrar al ir incrementando mientras se
           // lee cada byte
    if (firstByte & 128) { // This means the first byte has a value greater than
                           // 127, and so is beyond the ASCII range.
      if (firstByte &
          32) // This means that the first byte has a value greater than 191,
              // and so it must be at least a three-octet code point.
      {
        if (firstByte &
            16) // This means that the first byte has a value greater than 224,
...
void Marcador::imprimirCaracter(int caracter, int x, int y, int colorTexto,
                                int colorFondo) {
  switch (caracter) {
  case L',':
    caracter = 0x3c;
  ...
  // No uso ¿ porque la versión PS2 se compila con gcc3.2 que no soporta fichero
  // fuentes UTF8
  case L'\u00bf':
    caracter = 0x40;
    break;
  // case L'Ñ': caracter=0x57; break;
  // No uso Ñ porque la versión PS2 se compila con gcc3.2 que no soporta
  // ficheros fuentes UTF8
  case L'\u00d1':
    caracter = 0x57;
    break;
  case L'W':
  ...
  // TODO queda muy feo declarar esto en la funcion. Llevar a objetos estáticos
  // de la clase
  static const UINT8 c0[10] = {0x18, 0x0c, 0x00, 0x3c, 0x66,
                               0xc6, 0xfe, 0xc6, 0xe6, 0x66}; // À
  static const UINT8 c1[10] = {0x18, 0x30, 0x00, 0x3c, 0x66,
                               0xc6, 0xfe, 0xc6, 0xe6, 0x66}; // Á
  static const UINT8 c2[10] = {0x38, 0x6c, 0x00, 0x3c, 0x66,
                               0xc6, 0xfe, 0xc6, 0xe6, 0x66}; // Â
  static const UINT8 c3[10] = {0x36, 0x6c, 0x00, 0x3c, 0x66,
                               0xc6, 0xfe, 0xc6, 0xe6, 0x66}; // Ã
  static const UINT8 c4[10] = {0x00, 0x6c, 0x00, 0x3c, 0x66,
                               0xc6, 0xfe, 0xc6, 0xe6, 0x66}; // Ä
  static const UINT8 c7[10] = {0x00, 0x00, 0x00, 0x78, 0xc6,
                               0xc0, 0xe6, 0x7c, 0x18, 0x30}; //  Ç
  static const UINT8 c8[10] = {0x30, 0x18, 0x00, 0xdc, 0xe6,
                               0x60, 0x7c, 0x60, 0xe6, 0xdc}; // È
  static const UINT8 c9[10] = {0x18, 0x30, 0x00, 0xdc, 0xe6,
                               0x60, 0x7c, 0x60, 0xe6, 0xdc}; // É
  static const UINT8 ca[10] = {0x38, 0x6c, 0x00, 0xdc, 0xe6,
                               0x60, 0x7c, 0x60, 0xe6, 0xdc}; // Ê
  static const UINT8 cd[10] = {0x0c, 0x18, 0x00, 0x7e, 0x98,
                               0x30, 0x30, 0x30, 0x1a, 0xfc}; // Í
  static const UINT8 cf[10] = {0x00, 0x6c, 0x00, 0x7e, 0x98,
                               0x30, 0x30, 0x30, 0x1a, 0xfc}; // Ï
  // la d1 es la enye Ñ , que está en las fuentes normales en el lugar de la W,
  // que no se usaba en español
  static const UINT8 d2[10] = {0x30, 0x18, 0x00, 0x38, 0x6c,
                               0xc6, 0xc6, 0xc6, 0xee, 0x7c}; // Ò
  static const UINT8 d3[10] = {0x18, 0x30, 0x00, 0x38, 0x6c,
                               0xc6, 0xc6, 0xc6, 0xee, 0x7c}; // Ó
  static const UINT8 d5[10] = {0x36, 0x6c, 0x00, 0x38, 0x6c,
                               0xc6, 0xc6, 0xc6, 0xee, 0x7c}; // Ö
  static const UINT8 d6[10] = {0x00, 0x6c, 0x00, 0x38, 0x6c,
                               0xc6, 0xc6, 0xc6, 0xee, 0x7c}; // Õ
  static const UINT8 d9[10] = {0x18, 0x0c, 0x00, 0xe6, 0x66,
                               0xc6, 0xc6, 0xc6, 0xc6, 0x7c}; // Ù
  static const UINT8 da[10] = {0x18, 0x30, 0x00, 0xe6, 0x66,
                               0xc6, 0xc6, 0xc6, 0xc6, 0x7c}; // Ú

  static const UINT8 d1[10] = {0x00, 0x00, 0x00, 0x66, 0xe6,
                               0xc6, 0xd6, 0xd6, 0xfe, 0x66}; // w

  static const UINT8 ui[10] = {0x00, 0x00, 0x00, 0x00, 0x00,
                               0x00, 0x7e, 0xfc, 0x00, 0x00}; // -
  static const UINT8 uj[10] = {0x00, 0x00, 0x00, 0x30, 0x30,
  ...
  case 0xd1:
    data = d1;
    break; // la ñ se cambia por la w y la w por esta
  case L'-':
    data = ui;
    break;
  case L'\'':
  ...
  // a - en el original
  case L'#':
    caracter = '-'; // no hacer break, queremos que busque el grafico original
                    // asociado a -
  default: {
