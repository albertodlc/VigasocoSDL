...
void AccionesDia::ejecutaAccionesProgramadas() {
  ...
  if (laLogica->momentoDia == laLogica->oldMomentoDia)
    return;
...
void AccionesPrima::ejecuta(AccionesDia *ad) {
  ...
  if (laLogica->dia == 3) {
    ...
    // si guillermo no tiene el pergamino, se coloca en la habitación de detrás
    // del espejo
...
void AccionesDia::colocaPersonaje(Personaje *pers, int posX, int posY,
                                  int altura, int orientacion) {
  pers->posX = posX;
  pers->posY = posY;
  pers->altura = altura;
