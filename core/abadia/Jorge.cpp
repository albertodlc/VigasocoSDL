...
PosicionJuego Jorge::posicionesPredef[2] = {
    ...
    PosicionJuego(DERECHA, 0x19, 0x2b,
                  0x1a) // habitación iluminada de la biblioteca
};
...
//		0x00 -> estado normal
//		0x0b -> estado en el que se encuentra en la habitación de detrás
//del espejo cuando llega guillermo 		0x0c -> estado en el que ha dejado el libro
//para que lo lea guillermo 		0x0d -> estado en el que guillermo ha cogido el
//libro que le ha dado jorge 		0x0e -> estado en el que adso le informa a jorge
//que guillermo lleva guantes 		0x0f -> estado en el que jorge sale huyendo con el
//libro 		0x10 -> estado en el que jorge llega al sitio donde se muere y se
//completa la investigación 		0x1e -> estado en el que el abad está presentado a
//guillermo ante jorge 		0x1f -> estado en el que jorge habla con guillermo
//después de que hayan sido presentados
void Jorge::piensa() {
  ...
  if (laLogica->dia == 3) {
    ...
    if (laLogica->momentoDia == TERCIA) {
      ...
      // si el abad ha terminado de presentar a guillermo, éste le da la
      // bienvenida
      if (estado == 0x1f) {
        if (estaCerca(laLogica->guillermo)) {
          // pone en el marcador la frase SED BIENVENIDO, VENERABLE HERMANO; Y
          // ESCUCHAD LO QUE OS DIGO. LAS VIAS DEL ANTICRISTO SON LENTAS Y
          // TORTUOSAS. LLEGA CUANDO MENOS LO ESPERAS. NO DESPERDICIEIS LOS
          // ULTIMOS DIAS
      ...
      if (estaCerca(laLogica->guillermo)) {
        // pone en el marcador la frase VENERABLE JORGE, EL QUE ESTA ANTE VOS ES
        // FRAY GUILLERMO, NUESTRO HUESPED
  ...
  if (laLogica->dia >= 6) {
    if (estado == 0x0b) {
      // si ha terminado de decir la frase, deja el libro para que lo coja
      // guillermo
      if (!elGestorFrases->mostrandoFrase) {
    ...
    if (estado == 0x0c) {
      // si guillermo ha cogido el libro, le informa de que libro es y pasa al
      // siguiente estado
      if ((laLogica->guillermo->objetos & LIBRO) == LIBRO) {
        // pone en el marcador la frase ES EL COENA CIPRIANI DE ARISTOTELES.
        // AHORA COMPRENDEREIS POR QUE TENIA QUE PROTEGERLO. CADA PALABRA
        // ESCRITA POR EL FILOSOFO HA DESTRUIDO UNA PARTE DEL SABER DE LA
        // CRISTIANDAD. SE QUE HE ACTUADO SIGUIENDO LA VOLUNTAD DEL SEÑOR...
        // LEEDLO, PUES, FRAY GUILLERMO. DESPUES TE LO MOSTRATE A TI MUCHACHO
    ...
    if (estado == 0x0d) {
      ...
      if ((laLogica->guillermo->objetos & GUANTES) == 0) {
        ...
        if (!laLogica->haFracasado) {
          // si ha salido de la pantalla de detrás del espejo o ha terminado la
          // frase, mata a guillermo
          if ((elMotorGrafico->numPantalla == 0x72) ||
              (!elGestorFrases->mostrandoFrase)) {
        ...
        // si guillermo tenía los guantes y ha terminado de mostrar la frase,
        // adso le informa de que guillermo tenía puestos los guantes
        if (!elGestorFrases->mostrandoFrase) {
          // pone en el marcador la frase VENERABLE JORGE, VOIS NO PODEIS VERLO,
          // PERO MI MAESTRO LLEVA GUANTES.  PARA SEPARAR LOS FOLIOS TENDRIA QUE
          // HUMEDECER LOS DEDOS EN LA LENGUA, HASTA QUE HUBIERA RECIBIDO
          // SUFICIENTE VENENO
    ...
    if (estado == 0x0e) {
      if (!elGestorFrases->mostrandoFrase) {
        ...
        // pone en el marcador la frase FUE UNA BUENA IDEA ¿VERDAD?; PERO YA ES
        // TARDE
    ...
    if (estado == 0x10) {
      // si jorge ha llegado al sitio donde se muere y guillermo también, se ha
      // completado la investigación
      if ((aDondeHaLlegado == 1) && (elMotorGrafico->numPantalla == 0x67) &&
          (laLogica->guillermo->altura < 0x1e)) {
    ...
    // si entra a la habitación del espejo, inicia el estado de la secuencia
    // final
    if (elMotorGrafico->numPantalla == 0x73) {
      laLogica->bonus |= 0x0800;

      // pone en el marcador la frase SOIS VOS, GUILERMO... PASAD, OS ESTABA
      // ESPERANDO. TOMAD, AQUI ESTA VUESTRO PREMIO
