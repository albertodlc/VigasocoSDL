...
class BuscadorRutas : public Singleton<BuscadorRutas> {
  // campos
public:
  bool generadoCamino;       // indica si se ha generado algún camino en esta
                             // iteración del bucle principal
  bool seBuscaRuta;          // indica si se ejecuta el buscador de rutas o no
  int contadorAnimGuillermo; // contador de la animación de guillermo al inicio
                             // de esta iteración del bucle principal

  int numAlternativas;   // número de alternativas generadas
  int alternativaActual; // alternativa que se está probando actualmente
  int nivelRecursion; // nivel de recursión de la última ejecución del algoritmo
                      // de búsqueda de caminos
  int posXIni, posYIni; // datos sobre la posición final de las búsquedas
  int posXFinal, posYFinal,
      oriFinal; // datos sobre la posición final de las búsquedas

  RejillaPantalla *rejilla; // objeto para realizar operaciones relacionadas con
                            // la rejilla de pantalla

protected:
  INT32 *buffer;   // buffer para la búsqueda de caminos
  int lgtudBuffer; // longitud del buffer de búsqueda

  static UINT8 habitaciones[3][256]; // tabla con las habitaciones alcanzables
                                     // desde una habitación concreta
  static UINT8 habitacionesPuerta[6][4]; // tabla con las puertas y las
                                         // habitaciones que comunican
  static PosicionJuego
      alternativas[5]; // posiciones alternativas para la búsqueda del camino
  static int
      despOrientacion[4][2]; // tabla de desplazamientos según la orientación
  static int posDestinoOrientacion[4][2]; // tabla con las posiciones de destino
                                          // según la orientación

  int posPila;          // posición actual de la pila
  int posProcesadoPila; // posición de la pila mientras se examinan las
                        // alternativas

  FijarOrientacion
      *fijaPosOri[4]; // objetos para indicar las posiciones a las que hay que
                      // ir según la orientación a coger
...
public:
  ...
  bool esPosicionDestino(int posX, int posY, int altura, int alturaBase,
                         bool buscandoSolucion);
  void limpiaBitsBusquedaEnPantalla();

  // métodos de ayuda
protected:
  // métodos de más alto nivel relacionados con la búsqueda y generación de
  // rutas
  ...
  int generaCaminoAPosicionSiAlcanzable(PersonajeConIA *pers, int posXDest,
                                        int posYDest);
  ...
  bool esPantallaDestino(int posX, int posY, int numPlanta, int mascara,
                         int mascaraDestino);
