...
PosicionJuego Abad::posicionesPredef[10] = {
    PosicionJuego(ARRIBA, 0x88, 0x3c,
                  0x04), // posición en el altar de la iglesia
    ...
    PosicionJuego(ARRIBA, 0x88, 0x84,
                  0x02), // posición en la entrada de la abadía
    PosicionJuego(ABAJO, 0xa4, 0x58, 0x00), // posición de la primera parada
                                            // durante el discurso de bienvenida
    PosicionJuego(DERECHA, 0xa5, 0x21,
                  0x02), // posición para que entremos a nuestra celda
    PosicionJuego(
        DERECHA, 0x9c, 0x2a,
        0x02), // posición en la puerta de acceso de los monjes a la iglesia
    PosicionJuego(DERECHA, 0xc7, 0x27,
                  0x00), // posición en la pantalla en la que presenta a jorge
    PosicionJuego(ABAJO, 0x68, 0x61,
                  0x02), // posición en la puerta de la celda de severino
    PosicionJuego(DERECHA, 0x3a, 0x34,
                  0x0f) // posición a la entrada del pasillo por el que lleva a
                        // la biblioteca
};
...
//		0x00 -> estado en el que está esperando a que guillermo llegue a
//la abadía 		0x01 -> estado en el que va a la primera parada reproduciendo la
//frase 		0x02 -> estado en el que va a la primera parada pero ya ha terminado de
//reproducir la frase 		0x03 -> estado en el que va a la segunda parada
//reproduciendo la frase 		0x04 -> se va a su celda y al llegar avanza el momento
//del día 		0x05 -> cambia a este estado cuando está en vísperas para ir a misa
...
//		0x09 -> va hacia la puerta que comunica las celdas con la
//iglesia 		0x0a -> cierra la puerta que comunica las celdas con la iglesia,
//avanza el momento del día y se va a su celda 		0x0b -> busca a guillermo y le
//dice que debe abandonar la abadía 		0x0c -> estado en que el abad duerme 		0x0d ->
//estado en que el abad busca a guillermo para echarlo por la noche 		0x0e ->
//cambia a este estado cuando está en prima para ir a misa 		0x0f -> estado en el
//que se espera a que el abad termine la frase que le dice tras misa 		0x10 ->
//cambia a este estado cuando está en sexta para ir al refectorio, o después de
//que el abad le diga a guillermo una frase al terminar la misa 		0x11 -> estado
//al que se cambia cuando el abad le ha dicho a guillermo que se acerque después
//de misa 		0x12 -> estado al que se cambia cuando termina la frase de acercarse y
//se dice la frase que tenía pensada 		0x13 -> estado en el que el abad le ha
//dicho que hay que buscar a severino y van a su celda 		0x15 -> va a su celda,
//deja el pergamino y avanza el momento del día 		0x1f -> estado en el que va a la
//segunda parada pero ya ha terminado de reproducir la frase
// si el bit 7 del estado es 1 indica que está recriminandole algo a guillermo
void Abad::piensa() {
  // si guillermo visita el ala izquierda de la abadía el primer día o cuando es
  // prima, lo echa
  if ((laLogica->guillermo->posX < 0x60) &&
      ((laLogica->dia == 1) || (laLogica->momentoDia == PRIMA))) {
    estado = 0x0b;
  }

  // si guillermo visita la biblioteca cuando no es de noche, lo espera en la
  // entrada de la biblioteca y lo echa
  if ((laLogica->momentoDia >= PRIMA) &&
      (laLogica->guillermo->altura >= 0x16)) {
    aDondeVa = 9;
    estado = 0x0b;
  ...
  if (estado == 0x0b) {
    ...
    if (estaCerca(laLogica->guillermo)) {
      // si guillermo no ha muerto y se no se está reproduciendo ninguna frase,
      // lo echa
      if (!laLogica->haFracasado) {
        if (!elGestorFrases->mostrandoFrase) {
          // pone en el marcador la frase NO HABEIS RESPETADO MIS ORDENES.
          // ABANDONAD PARA SIEMPRE ESTA ABADIA
  ...
  if ((elMotorGrafico->numPantalla == 0x0d) &&
      (laLogica->opcionPersonajeCamara == 0)) {
    // si está cerca de guillermo le recrimina que haya entrado en su celda y le
    // expulsa
    if (estaCerca(laLogica->guillermo)) {
      aDondeVa = POS_GUILLERMO;
  ...
  if ((aDondeHaLlegado == aDondeVa) && (aDondeHaLlegado == 2) &&
      ((objetos & PERGAMINO) == PERGAMINO)) {
    laLogica->pergaminoGuardado = true;

    // modifica la máscara de los objetos que puede coger para no coger el
    // pergamino otra vez
    mascaraObjetos = 0;
    ...
    // si está en el estado 0x15 y no tiene el pergamino, cambia de estado y
  ...
  // si está en visperas, va a la iglesia y espera a que todos estén en su sitio
  // para avanzar el momento del día
  if (laLogica->momentoDia == VISPERAS) {
    ...
    // comprueba si los personajes con IA están en su sitio para la misa de
    // vísperas
    compruebaPosMonjesEnIglesiaEnVisperas(laLogica->dia);

    // espera a que el abad, guillermo y los personajes con IA estén en su sitio
    // para comenzar la misa o la comida
    esperaParaComenzarActo();

    return;
  }

  // si está en prima, va a la iglesia y espera a que todos estén en su sitio
  // para avanzar el momento del día
  if (laLogica->momentoDia == PRIMA) {
    estado = 0x0e;
  ...
  // si berengario nos ha dicho que guillermo ha cogido el pergamino, va a por
  // guillermo
  if (guillermoHaCogidoElPergamino) {
    aDondeVa = POS_GUILLERMO;

    // si el abad le ha quitado el pergamino a guillermo, cambia de estado para
    // dejarlo en su celda
    if ((objetos & PERGAMINO) == PERGAMINO) {
      ...
      if (estaCerca(laLogica->guillermo)) {
        ...
        // pone el contador al nivel máximo para que le llame la atención a
        // guillermo
        contador = 0xc9;
      }
  ...
  if (laLogica->momentoDia == COMPLETAS) {
    // si ha terminado la misa y no se está reproduciendo ninguna frase, se
    // marcha a la entrada de la celda de guillermo
    if (estado == 0x06) {
      if (!elGestorFrases->mostrandoFrase) {
        contador = 0;
        aDondeVa = 5;
    ...
    if (estado == 0x07) {
      ...
      if (elMotorGrafico->numPantalla == 0x3e) {
        ...
        // si está cerca guillermo, le dice que entre en su celda y pasa al
        // estado 0x08
        if (estaCerca(laLogica->guillermo)) {
          estado = 0x08;
    ...
    if (estado == 0x09) {
      ...
      if (elMotorGrafico->numPantalla == 0x3e) {
        ...
        // si guillermo ha salido de su celda, cambia al estado 0x08 y va a por
        // él
  ...
  if (laLogica->momentoDia == NOCHE) {
    ...
    // si ha llegado a su celda después de cerrar la puerta, inicia el contador
    // y cambia de estado
    ...
    if (estado == 0x0c) {
      ...
      if (laLogica->guillermo->posX >= 0x60) {
        ...
        // si el contador ha sobrepasado el límite o es el quinto día y hemos
        // cogido la llave del abad, se despierta
        if ((contador >= 0xfa) ||
            ((laLogica->dia == 5) &&
             ((laLogica->guillermo->objetos & LLAVE1) == LLAVE1))) {
          estado = 0x0d;
        }
      }
    ...
    if (estado == 0x0d) {
      // si guillermo está en el ala izquierda de la abadía o en su celda, se
      // vuelve a dormir
      if ((laLogica->guillermo->posX < 0x60) ||
          (elMotorGrafico->numPantalla == 0x3e)) {
        estado = 0x0c;
        contador = 0x32;
      } else {
  ...
  if (laLogica->dia == 1) {
    if (laLogica->momentoDia == NONA) {
      ...
      if (estado == 0x00) {
        // si guillermo está cerca del abad, le da la bienvenida y le dice que
        // le siga
        if (estaCerca(laLogica->guillermo)) {
          // muestra la frase BIENVENIDO A ESTA ABADIA, HERMANO. OS RUEGO QUE ME
          // SIGAIS. HA SUCEDIDO ALGO TERRIBLE
      ...
      if (estaCerca(laLogica->guillermo)) {
        ...
        if (estado == 0x01) {
          // si va a la primera parada y no se está reproduciendo ninguna voz,
          // cambia al estado 0x02
          if ((aDondeVa == 4) && (!elGestorFrases->mostrandoFrase)) {
            estado = 0x02;
          } else if (!elGestorFrases->mostrandoFrase) {
            // si ha terminado la primera frase, dice la segunda y se va a la
            // primera parada

            // muestra la frase TEMO QUE UNO DE LOS MONJES HA COMETIDO UN
            // CRIMEN. OS RUEGO QUE LO ENCONTREIS ANTES DE QUE LLEGUE BERNARDO
            // GUI, PUES	NO DESEO QUE SE MANCHE EL NOMBRE DE ESTA ABADIA
        ...
        if (estado == 0x02) {
          ...
          // si ha llegado a la primera parada y no está reproduciendo una
          // frase, pasa al estado 0x03
        ...
        if (estado == 0x03) {
          ...
          if ((aDondeVa == 5) && (!elGestorFrases->mostrandoFrase)) {
            ...
            // si ha terminado la segunda frase, dice la tercera y se va a la
            // segunda parada

            // muestra la frase DEBEIS RESPETAR MIS ORDENES Y LAS DE LA ABADIA.
            // ASISTIR A LOS OFICIOS Y A LA COMIDA. DE NOCHE DEBEIS ESTAR EN
            // VUESTRA CELDA
        ...
        if (estado == 0x1f) {
          ...
          // si ha llegado a la segunda parada y no está reproduciendo una
          // frase, pasa al estado 0x04 y se despide
  ...
  if (laLogica->dia == 2) {
    // DEBEIS SABER QUE LA BIBLIOTECA ES UN LUGAR SECRETO. SOLO MALAQUIAS PUEDE
    // ENTRAR. PODEIS IROS
    numFrase = 0x16;
  ...
  if (laLogica->dia == 3) {
    ...
    if ((estado == 0x10) && (laLogica->momentoDia == TERCIA)) {
      ...
      if (estaCerca(laLogica->guillermo)) {
        ...
        // si han llegado a donde estaba jorge, reprime a guillermo y sigue por
        // donde lo dejó
  ...
  if (laLogica->dia == 5) {
    if (laLogica->momentoDia == NONA) {
      ...
      if (aDondeHaLlegado == 8) {
        ...
        contador++;
        // si se ha esperado suficiente, cambia de estado e informa de la muerte
        // de severino
        if (contador >= 0x1e) {
          estado = 0x10;

          // muestra la frase DIOS SANTO... HAN ASESINADO A SEVERINO Y LE HAN
          // ENCERRADO
        ...
        if ((aDondeVa == 8) || (estado == 0x13)) {
          ...
          if (estado == 0x13) {
            // si está cerca de guillermo, va a la celda de severino. En otro
            // caso, le recrimina
            if (estaCerca(laLogica->guillermo)) {
              aDondeVa = 8;
            } else {
          ...
          // muestra la frase VENID, FRAY GUILLERMO, DEBEMOS ENCONTRAR A
          // SEVERINO
  ...
  if (aDondeHaLlegado == aDondeVa) {
    ...
    if (lleganLosMonjes == 0) {
      ...
      if (guillermoBienColocado >= 0x01) {
        // si se ha superado el contador de puntualidad, lo reinicia y reprende
        // a guillermo
        if (contador >= 0x32) {
          contador = 0;
          ...
          if (!elGestorFrases->mostrandoFrase) {
            ...
            if (guillermoBienColocado == 2) {
              ...
              // si el contador pasa el límite tolerable, le indica que ocupe su
              // sitio
              if (contador >= 0x1e) {
                contador = 0;
              ...
              // si guillermo está en su sitio, el abad habla y se avanzará el
              // día cuando termine de hablar
              elGestorFrases->muestraFrase(numFrase);
              laLogica->avanzarMomentoDia = true;
            }
          }

          // si se tenía que avanzar el momento del día pero guillermo se ha
          // movido del sitio, le reprime
          if ((laLogica->avanzarMomentoDia == true) &&
...
void Abad::compruebaPosGuillermo(int posX, int posY, int orientacion) {
  ...
  if (laLogica->guillermo->altura >= 0x0b)
    return;
  ...
  // si está en la posición de destino y con la orientación deseada, sale
  // devolviendo 1
...
// comprueba que todos los personajes con IA estén en su sitio para la misa de
// vísperas
void Abad::compruebaPosMonjesEnIglesiaEnVisperas(int numDia) {
  ...
  // en otro caso, comprueba que los monjes que deben estar presentes hayan
...
void Abad::compruebaPosMonjesEnIglesiaEnPrima(int numDia) {
  ...
  // comprueba que los monjes que deben estar presentes hayan llegado a la
  // iglesia y cambia la frase a mostrar
...
void Abad::paseaPorLaAbadia() {
  ...
  if ((laLogica->malaquias->aDondeVa == POS_ABAD) ||
      (laLogica->berengario->aDondeVa == POS_ABAD) ||
      (laLogica->bernardo->aDondeVa == POS_ABAD)) {
    ...
    if (aDondeHaLlegado == aDondeVa) {
      ...
      // si bernardo tiene el pergamino, va a la entrada de la abadía a
      // esperarle
...
void Abad::diceFraseAGuillermoEnTercia() {
  ...
  // si ha terminado la frase que le indicaba que se acercase, comienza a decir
  // la frase que tenía pensada
  if (estado == 0x12) {
    estado = 0x0f;
    aDondeVa = 0;
