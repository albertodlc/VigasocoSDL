...
//
//	Clase que se encarga de generar los bloques que forman las pantallas,
// calcular la zona que 	tapa el bloque y grabar toda esa información en
// la capas que forman el buffer de tiles. 	Además, esta clase se encarga de
// dibujar las pantallas una vez que se ha rellenado el buffer 	de tiles.
//
...
public:
  ...
  // pero con VGA hay imperfecciones visuales bastante notables (ver la pantalla
  // en la iglesia justo antes de llegar a la pantalla del altar. Hay un pulpito
  // al que se llega por unas escaleras, en el que queda un hueco sin dibujar
  // que queda con el color de fondo de la pantalla !!!
  //  En el remake 32 bits 256 colores el pulpito se ve bien, pero en el suelo
  //  al empezar las escaleras, se mezclan 2 tipos de suelos ¿ cual es el bueno
  //  ? por que en la pantalla anterior, en la otra vista, el suelo es gris
  //  completo antes de empezar las escaleras ..., pero en esta pantalla salen
  //  losas cuadradas ...
  //
  static const int nivelesProfTiles =
      4; // Con esto ya se ve bien casi todo, incluso a veces mejor al remake 32
         // bits 256 colores static const int nivelesProfTiles = 5; // No se
         // nota diferencia con el anterior ...
...
public:
  struct TileInfo {
    UINT8 profX[nivelesProfTiles]; // profundidad del tile en x (en coordenadas
                                   // locales)
    UINT8 profY[nivelesProfTiles]; // profundidad del tile en y (en coordenadas
                                   // locales)
...
public:
  ...
  int mascaras[4]
              [4]; // tablas de máscaras and y or para cada uno de los colores

  UINT8 *datosPantalla; // puntero a los datos que forman la pantalla
  int comandosBloque;   // desplazamiento a los datos de los comandos que forman
                        // un bloque
  int datosBloque[17]; // buffer donde guarda los datos para construir el bloque
                       // actual
  ...
  int estadoOpsX[4];       // usado para cambiar el sentido de las x en algunas
                           // operaciones
...
public:
  ...
  void iniciaInterpretacionBloque(UINT8 *tilesBloque, bool modificaTiles,
                                  int altura);
