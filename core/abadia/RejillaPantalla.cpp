...
// dada la posición de un personaje, calcula los mínimos valores visibles del
// área de juego
...
// dado un personaje, rellena la rejilla con la información de altura de la
// planta recortada para la pantalla
void RejillaPantalla::rellenaAlturasPantalla(Personaje *pers) {
  ...
  // obtiene los mínimos valores visibles para la pantalla en la que se
  // encuentra el personaje
  calculaMinimosValoresVisibles(pers);

  // halla el desplazamiento a los datos de la altura para la planta en la que
  // se encuentra el personaje
  int datosAlturaPlanta[] = {0x18a00, 0x18f00, 0x19080};
  UINT8 *datosAltura =
      &roms[datosAlturaPlanta[motor->obtenerPlanta(minAltura)]];
  ...
  while ((*datosAltura) != 0xff) {
    ...
    // si la entrada no es de 5 bytes, la longitud se codifica en 4 bits en vez
    // de en 8
    ...
    // si llega hasta aquí, alguna parte del bloque es visible, por lo que
    // modifica el buffer de alturas

    // según el tipo de bloque, fija los datos de la altura
    if ((tipoBloque & 0x07) != 5) {
      static int incrementos[4][2] = {{1, 0}, {0, -1}, {-1, 0}, {0, 1}};
      ...
      if (distX < 0) {
        ...
        // si el bloque es más grande que la zona visible, se recorta en
        // longitud
        if ((distX + lgtudX) > 24) {
          lgtudX = 24;
        } else {
        ...
        // si el bloque es más grande que la zona visible, recorta la longitud
        // del bloque
        if ((distX + lgtudX) > 24) {
...
// comprueba si la posición que se le pasa (en coordenadas de mundo) está dentro
// de las 20x20 posiciones centrales de la rejilla y si es así, devuelve la
// posición en el sistema de coordenadas de la rejilla
bool RejillaPantalla::ajustaAPosRejilla(int posX, int posY, int &posXRejilla,
                                        int &posYRejilla) {
...
// comprueba si la posición que se le pasa está en las 20x20 posiciones
// centrales de la rejilla de la pantalla actual, y de ser así, se devuelve su
// posición en el sistema de coordenadas de la rejilla
bool RejillaPantalla::estaEnRejillaCentral(PosicionJuego *pos, int &posXRejilla,
                                           int &posYRejilla) {
  ...
  // si la posición no está en las 20x20 posiciones centrales de la rejilla,
  // sale
...
// devuelve la diferencia de altura y posición del personaje si sigue avanzando
// hacia donde mira
bool RejillaPantalla::obtenerAlturaPosicionesAvance(Personaje *pers,
                                                    int &difAltura1,
                                                    int &difAltura2,
                                                    int &avanceX,
                                                    int &avanceY) {
  // si el personaje no está en la misma planta que la de la rejilla, sale
  if (elMotorGrafico->obtenerAlturaBasePlanta(pers->altura) != minAltura)
    return false;

  // obtiene la altura relativa con respecto a esta planta
  int alturaLocal =
      pers->altura - elMotorGrafico->obtenerAlturaBasePlanta(pers->altura);

  return obtenerAlturaPosicionesAvanceComun(pers, alturaLocal, difAltura1,
                                            difAltura2, avanceX, avanceY);
}

// devuelve la diferencia de altura y posición del personaje si sigue avanzando
// hacia donde mira
bool RejillaPantalla::obtenerAlturaPosicionesAvance2(Personaje *pers,
                                                     int &difAltura1,
                                                     int &difAltura2,
                                                     int &avanceX,
                                                     int &avanceY) {
  return obtenerAlturaPosicionesAvanceComun(pers, 0, difAltura1, difAltura2,
                                            avanceX, avanceY);
}
...
// devuelve la diferencia de altura y posición del personaje si sigue avanzando
// hacia donde mira
bool RejillaPantalla::obtenerAlturaPosicionesAvanceComun(
    Personaje *pers, int alturaLocal, int &difAltura1, int &difAltura2,
    int &avanceX, int &avanceY) {
  int posXLocal, posYLocal;

  // si la posición no está dentro de las 20x20 posiciones centrales de la
  // pantalla que se muestra, sale
  if (!estaEnRejillaCentral(pers, posXLocal, posYLocal))
    return false;
  ...
  // rellena el buffer para el cálculo del avance con las posiciones relevantes
  ...
  for (int j = 0; j < 4; j++) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (alturaPos < 0x10) {
        // si no hay un personaje en esa posición, obtiene la diferencia de
        // altura entre la posición y el personaje
  ...
  if (!pers->enDesnivel) {
    ...
    // si en las 2 posiciones hacia las que quiere avanzar el personaje no hay
    // la misma altura
    ...
    // si el personaje ocupa una posición en la rejilla, guarda la diferencia de
    // altura de las 2 posiciones hacia las que quiere avanzar
    difAltura1 = bufCalculoAvance[1][1];
    difAltura2 = bufCalculoAvance[0][1];
  }

  // guarda el avance en cada coordenada según la orientación en la que se
  // quiere avanzar
