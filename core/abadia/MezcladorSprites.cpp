...
/////////////////////////////////////////////////////////////////////////////
MezcladorSprites::MezcladorSprites(GeneradorPantallas *generador, UINT8 *buffer,
                                   int lgtudBuffer) {
  genPant = generador;
...
void MezcladorSprites::mezclaSprites(Sprite **sprites, int num) {
  ...
  if (numSpritesRedib == 0)
    return;

  int i = 1;
  bool huboIntercambio;

  // ordena los sprites ascendentemente según su profundidad usando el método de
  // la burbuja mejorado
  do {
    ...
    for (int j = numSprites - 1; j >= i; j--) {
      if (sprites[listaSprites[j - 1]]->profundidad >
          sprites[listaSprites[j]]->profundidad) {
  ...
  // recorre los sprites de la lista, calculando el área que debe dibujarse y
  // mezclándolos en el buffer de sprites
  for (int i = 0; i < numSprites; i++) {
    ...
    // ajusta las dimensiones del area a redibujar para que abarque el sprite
    // antiguo
    spr->ampliaDimViejo();

    // calcula el desplazamiento correspondiente a la posición inicial en el
    // buffer de tiles. las coordenadas del sprite (32, 40) se corresponden al
    // primer pixel del buffer de tiles
    ...
    // si no hay sitio para el sprite en el buffer de sprites, vuelca a pantalla
    // los sprites procesados y repite el proceso con el resto
    ...
    memset(&bufferMezclas[spr->despBuffer], 0,
           spr->anchoFinal * spr->altoFinal * 4);

    // empieza a dibujar desde la mínima profundidad
    int profMinX = 0, profMinY = 0;

    // recorre los sprites de la lista, mezclando la parte que se vea con el
    // sprite que se está procesando
    for (int j = 0; j < numSprites; j++) {
      ...
      if (sprAMezclar->desaparece)
        continue;

      int lgtudClipX, lgtudClipY, dist1X, dist2X, dist1Y, dist2Y;

      // comprueba si el sprite a mezclar puede verse en la zona del sprite que
      // se está dibujando
      if (!recortaSprite(spr->posXTile, spr->anchoFinal, sprAMezclar->posXPant,
                         sprAMezclar->ancho, lgtudClipX, dist1X, dist2X)) {
        continue;
      }

      // comprueba si el sprite a mezclar puede verse en la zona del sprite que
      // se está dibujando
      if (!recortaSprite(spr->posYTile, spr->altoFinal, sprAMezclar->posYPant,
                         sprAMezclar->alto, lgtudClipY, dist1Y, dist2Y)) {
        continue;
      }

      // si llega aquí es porque alguna parte del sprite es visible

      // vuelca al buffer de sprites los tiles no dibujados que están detrás del
      // sprite actual
      ...
      // dibuja la parte visible del sprite que se está mezclando en el área
      // ocupada por el sprite actual
      sprAMezclar->dibuja(spr, bufferMezclas, lgtudClipX, lgtudClipY, dist1X,
                          dist2X, dist1Y, dist2Y);
    }

    // si falta algún tile por superponer al sprite, lo dibuja y limpia las
    // marcas del buffer de tiles
    dibujaTilesEntreProfundidades(spr, profMinX, profMinY, 0xfd, 0xfd,
                                  spr->despBuffer, true);
  }
...
// vuelca al buffer de sprites los tiles no dibujados que están detrás del
// sprite actual
void MezcladorSprites::dibujaTilesEntreProfundidades(Sprite *spr, int profMinX,
                                                     int profMinY, int profMaxX,
                                                     int profMaxY, int desp,
                                                     bool ultimaPasada) {
  ...
  for (int j = 0; j < numTilesY; j++) {
    ...
    for (int i = 0; i < numTilesX; i++) {
      ...
      if (estaEnBufferTiles(bufTilesPosX + i, bufTilesPosY + j)) {
        ...
        // inicialmente en esta llamada no se ha pintado en esta posición del
        // buffer de tiles
        ...
        for (int k = 0; k < GeneradorPantallas::nivelesProfTiles; k++) {
          ...
          // si en esta llamada no se ha pintado en esta posición del buffer de
          // tiles, comprueba si hay que pintar el tile que hay en esta capa de
          // profundidad. Si se ha pintado en esta llamada y el tile de esta
          // capa se había pintado en otra iteración anterior, lo combina sin
          ...
          // si el tile está entre los límites de profundidad y no se ha
          // dibujado todavía
          if (visible && ((ti->profX[k] & 0x80) == 0)) {
            // marca el tile como dibujado
            ti->profX[k] |= 0x80;

            // indica que en esta llamada ha pintado algún tile para esta
            // posición del buffer de tiles
            haPintado = true;
                ...
                for (int j = 0; j < 8; j++){
                        ...
                        for (int i = 0; i < 4; i++){
                                ...
                                for (int k = 0; k < 4; k++){
                                        ...
                                        *dest = cpc6128->unpackPixelMode1(data,
k); dest++;
                                }
                        ...
                        // avanza a la siguiente línea del sprite en el buffer
de sprites dest += despSgteLinea;
                }
        } else {
...
cpc6128->unpackPixelMode1(data, k);
                                        ...
                                        // obtiene el color del pixel en el
buffer de sprites int oldColor = *dest;

                                        // combina el color del pixel del buffer
de sprites con el nuevo *dest = (oldColor & genPant->mascaras[numTabla +
1][color]) | genPant->mascaras[numTabla][color]; dest++;
                                }
...
void MezcladorSprites::combinaTile(Sprite *spr, int tile, int despBufSprites) {
  ...
  UINT8 *tileDataVGA =
      &roms[0x24000 - 1 - 0x4000 + 0x1400 + tile * 8 * 16]; // VGA
    ...
    for (int j = 0; j < 8; j++) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        for (int k = 0; k < 4; k++) {
          ...
          //*dest = (oldColor & genPant->mascaras[numTabla + 1][color]) |
          //genPant->mascaras[numTabla][color]; printf("*dest %d oldColor %d
          // color %d numTabla %d\n",*dest,oldColor,color,numTabla); *dest = 0;
          //// se pinta en azul, el color del fondo *dest = 1;  // se pinta en
          //naranja *dest = 2; // amarillo claro
          // *dest= 3; // negro
          dest++;
        }
...
void MezcladorSprites::postProcesaSprites(Sprite **sprites, int num) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    // si el sprite es de un objeto que desaparece, marca el sprite como
    // inactivo
...
void MezcladorSprites::vuelcaBufferAPantalla(Sprite *spr) {
  ...
  // comprueba la distancia en y desde el inicio del sprite al inicio de la
  // pantalla
  int distY = posY - 40;
  ...
  // comprueba la distancia en x desde el inicio del sprite al inicio de la
  // pantalla
  int distX = posX - (32 / 4);
...
// comprueba si el sprite a mezclar puede verse en la zona del sprite que se
// está dibujando si es así lo recorta al área que se está dibujando y devuelve
// true. En otro caso, devuelve false
bool MezcladorSprites::recortaSprite(int posVis, int lgtudVis, int pos,
                                     int lgtud, int &lgtudClip, int &dist1,
                                     int &dist2) {
  ...
  if (dif == 0) {
    ...
    // si la longitud del sprite original es mayor que la longitud del sprite a
    // mezclar, recorta la longitud
    if (lgtudVis > lgtud) {
      lgtudClip = lgtud;
    }
  } else if (dif > 0) {
    ...
    // si el sprite a mezclar termina antes de que empiece el sprite original,
    // queda fuera del área visible
    if (dif >= lgtud) {
      return false;
    }
    ...
    // si el sprite a mezclar termina antes de que lo haga el sprite original,
    // recorta la longitud
    if ((dif + lgtudVis) >= lgtud) {
      lgtudClip = lgtud - dif;
    } else {
      // el sprite a mezclar ocupa toda la zona del sprite original, por lo que
      // recorta la longitud
      lgtudClip = lgtudVis;
    }
  } else {
    ...
    // si el sprite a mezclar empieza después de que termine el sprite original,
    // queda fuera del área visible
    if (dif >= lgtudVis) {
      return false;
    }
    ...
    dist1 =
        dif; // distancia de la posición original al inicio del sprite a mezclar
    dist2 = 0;

    // si el sprite a mezclar no termina antes de que lo haga el sprite
    // original, recorta la longitud
