...
public:
  ...
  GeneradorPantallas *genPant; // generador de pantallas interpretando los
                               // bloques de construcción
  MezcladorSprites
      *mezclador; // mezclador de los sprites con el resto de la pantalla
  ...
  int alturaBasePantalla; // altura base de la planta que se muestra en la
                          // pantalla actual
  ...
  static int
      tablaDespOri[4][2]; // tabla con los desplazamientos según la orientación
  static UINT8 plantas[3][256]; // mapa de las plantas de la abadía

protected:
  UINT8 *roms; // puntero a los datos del juego

  // objetos para la transformación de coordenadas según el tipo de cámara de la
  // pantalla
...
public:
  ...
  int actualizaCoordCamara(EntidadJuego *entidad, int &posXPant, int &posYPant,
                           int &sprPosY);
