...
//
//	Cuando se llama al método muestraInfo() se sobreescribe parte de la
//pantalla con mapas 	del juego. En la esquina superior derecha se muestra el
//mapa de alturas para la 	pantalla que sigue la cámara. A la derecha del mapa de
//alturas se muestra el mapa de 	alturas completo de la planta en la que está el
//personaje al que sigue la cámara. 	Debajo del mapa de alturas de la pantalla
//actual se muestran los mapas de las plantas 	en las que no se encuentra el
//personaje.
//
//	Como actualmente VIGASOCO no soporte múltiples bitmpaps desde distintos
//threads, no 	hay más remedio que dibujar encima del bitmap principal, por lo
//que aunque se desactive 	parte de la información que se muestra, la zona de
//juego estará sucia hasta que no se 	redibuje, y la parte que no cae en la zona
//de juego no se limpiará. Cuando VIGASOCO 	soporte múltiples bitmaps en
//distintos threads esto se corregirá.
//
/////////////////////////////////////////////////////////////////////////////
...
protected:
  ...
  bool mostrarLogica;  // indica si se muestran los datos relacionados con la
                       // lógica
  bool mostrarRejilla; // indica si se muestra la rejilla de pantalla
  bool mostrarMapaPlantaActual; // indica si se muestra el mapa de la planta
                                // actual
  bool mostrarMapaRestoPlantas; // indica si se muestra el mapa de las plantas
                                // en las que no estamos

  CPC6128 *
      cpc6128; // objeto de ayuda para realizar operaciones gráficas del cpc6128
  UINT8 *alturasPlanta[3]; // array de punteros a los buffers donde se almacenan
                           // las alturas de cada planta
...
protected:
  ...
  void dibujaAlturaPosicionPlanta(int posX, int posY, int x, int y, int zoom,
                                  UINT8 *alturas);
  ...
  void dibujaPixelCuadradoZoom(int posX, int posY, int i, int j, int zoom,
                               int color);
