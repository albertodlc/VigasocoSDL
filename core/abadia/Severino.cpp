...
PosicionJuego Severino::posicionesPredef[4] = {
    ...
    PosicionJuego(DERECHA, 0xc9, 0x2a,
                  0x00) // pantalla de al lado de las celdas de los monjes
};
...
//		bit 0 -> indica que guillermo está en el ala izquierda de la
//abadía 		bit 1 -> indica que ha terminado de presentarse ante guillermo 		bit 2 ->
//indica que se ha presentado ante guillermo
void Severino::piensa() {
  ...
  case PRIMA:
    ...
    if (elGestorFrases->mostrandoFrase && (aDondeVa == POS_GUILLERMO))
      return;

    // en otro caso, va a la iglesia
    aDondeVa = 0;

    // si es el quinto día y guillermo no está en el ala izquierda de la abadía,
    // va a por él
    if ((laLogica->dia == 5) && ((estado & 0x01) == 0)) {
      if (laLogica->guillermo->posX < 0x60) {
        ...
        // si alcanza a guillermo le dice la frase ESCUCHAD HERMANO, HE
        // ENCONTRADO UN EXTRAÑO LIBRO EN MI CELDA
  ...
  case TERCIA:
  case NONA:
    // a partir del segundo día, si severino no va a su celda y no se ha
    // presentado
    if (((estado & 0x02) == 0) &&
        ((aDondeHaLlegado >= 2) || (aDondeHaLlegado == POS_GUILLERMO)) &&
        (laLogica->dia >= 2) && (laLogica->abad->aDondeVa != POS_GUILLERMO)) {
      ...
      if (((estado & 0x04) == 0) && (!elGestorFrases->mostrandoFrase)) {
        ...
        if (estaCerca(laLogica->guillermo)) {
          ...
          // pone en el marcador la frase VENERABLE HERMANO, SOY SEVERINO, EL
          // ENCARGADO DEL HOSPITAL. QUIERO ADVERTIROS QUE EN ESTA ABADIA
          // SUCEDEN COSAS MUY EXTRAÑAS. ALGUIEN NO QUIERE QUE LOS MONJES
          // DECIDAN POR SI SOLOS LO QUE DEBEN SABER
    ...
    // si está junto a guillermo y no se está reproduciendo ninguna frase, le
    // cuenta las manchas de berengario
    if (aDondeHaLlegado == POS_GUILLERMO) {
      if (!elGestorFrases->mostrandoFrase) {
        // pone en el marcador la frase ES MUY EXTRAÑO, HERMANO GUILLERMO.
        // BERENGARIO TENIA MANCHAS NEGRAS EN LA LENGUA Y EN LOS DEDOS
    ...
    if (aDondeHaLlegado == 2) {
      ...
      // en otro caso, va a la habitación que está al lado de las celdas de los
      // monjes
      aDondeVa = 3;
