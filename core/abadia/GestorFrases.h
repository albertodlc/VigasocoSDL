...
class GestorFrases : public Singleton<GestorFrases> {
  // campos
public:
  volatile bool mostrandoFrase; // copia de reproduciendoFrase para que el otro
                                // thread consulte el estado de las frases

  std::string frasePergamino; // frase del pergamino (necesario, porque en C++
                              // no se puede modificar un static char *)

protected:
  CPC6128 *cpc6128; // objeto que presta ayuda para realizar operaciones
                    // gráficas del cpc6128

  int contadorActualizacion; // contador para actualizar la frase que se está
                             // poniendo en el marcador
  int espaciosParaFin;     // número de espacios para que la frase haya salido
                           // completamente del marcador
  bool fraseTerminada;     // indica si se terminó una frase
  bool reproduciendoFrase; // indica que se está mostrando una frase en el
                           // marcador
