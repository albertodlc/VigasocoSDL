...
UINT16 PersonajeConIA::comandosGirar[4][2] = {
    {0x8000,
     1}, // avanzar una posición hacia delante (no debería llamarse nunca)
...
UINT16 PersonajeConIA::comandosAvanzar[5][2] = {
    {0x8000,
     2}, // si está girado, avanza, en otro caso sube (y sigue ocupando una
         // posición) (sólo usado si el personaje ocupa una posición)
...
int PersonajeConIA::distanciasOri[4][4] = {
    {6, 24, 6, 12}, {6, 12, 12, 24}, {12, 24, 6, 12}, {6, 12, 6, 24}};
...
// método llamado desde el bucle principal para que el personaje interactue con
// el mundo virtual
void PersonajeConIA::run() {
  ...
  // si se han terminado los comandos de movimiento, genera los comandos para ir
  // a donde se quiere
  elBuscadorDeRutas->generaAccionesMovimiento(this);
}

// interpreta los comandos almacenados en el buffer de acciones para moverse a
// donde quiere
void PersonajeConIA::ejecutaMovimiento() {
  ...
  // en otro caso hay que avanzar. Como sólo tenemos calculada la rejilla de la
  // pantalla que muestra el motor, si el personaje que quiere avanzar no está
  // en la pantalla que se muestra, hacemos trampa e indicamos que el movimiento
  // va a poder realizarse con éxito. Si el personaje está en la pantalla que se
  // está mostrando actualmente, el movimiento se realiza de forma precisa y sin
  // hacer trampas

  // obtiene la altura de las posiciones hacia las que se va a mover
  if (!elMotorGrafico->rejilla->obtenerAlturaPosicionesAvance(
          this, difAltura1, difAltura2, avanceX, avanceY)) {
    // si llega aquí es porque el personaje no está en la pantalla que muestra
    // la cámara
...
int PersonajeConIA::leeBit() {
  // si se han terminado los bits de la accion actual, obtiene un nuevo byte de
  // comandos
  if (numBitAcciones == 0) {
...
void PersonajeConIA::avanzaPosicion(int difAltura1, int difAltura2, int avanceX,
                                    int avanceY) {
  int numEntrada;
  bajando = false;
  ...
  escribeComandos(comandosAvanzar[numEntrada][0],
                  comandosAvanzar[numEntrada][1]);
}
...
// descarta los movimientos pensados e indica que hay que pensar un nuevo
// movimiento
...
// escribe los bits de los comandos que se le pasa en el buffer de acciones del
// personaje
...
bool PersonajeConIA::estaCerca(Personaje *pers) {
  ...
  if (elMotorGrafico->obtenerAlturaBasePlanta(altura) !=
