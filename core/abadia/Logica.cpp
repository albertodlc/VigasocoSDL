...
void Logica::compruebaLecturaLibro() {
  ...
  if ((guillermo->objetos & LIBRO) == 0)
    return;

  // si guillermo tiene los guantes, sale
  if ((guillermo->objetos & GUANTES) != 0)
    return;
  ...
  if ((cntLeeLibroSinGuantes & 0xff) == 0) {
    ...
    // escribe en el marcador la frase: ESTAIS MUERTO, FRAY GUILLERMO, HABEIS
    // CAIDO EN LA TRAMPA
...
void Logica::actualizaBonusYCamara() {
  ...
  if (losControles->estaSiendoPulsado(KEYBOARD_1)) {
    ...
    return;
  } else if (losControles->estaSiendoPulsado(KEYBOARD_2)) {
    opcionPersonajeCamara = 5; // severino
    return;
  } else if (losControles->estaSiendoPulsado(KEYBOARD_3)) {
    opcionPersonajeCamara = 2; // malaquias
    return;
  } else if (losControles->estaSiendoPulsado(KEYBOARD_4)) {
    opcionPersonajeCamara = 4; // berengario
    return;
  } else if (losControles->estaSiendoPulsado(KEYBOARD_5)) {
    opcionPersonajeCamara = 6; // jorge
    return;
  } else if (losControles->estaSiendoPulsado(KEYBOARD_6)) {
    opcionPersonajeCamara = 7; // bernardo
    return;
  } else if (losControles->estaSiendoPulsado(KEYBOARD_7)) {
  ...
  if (((berengario->aDondeVa == POS_LIBRO) && (berengario->posX < 0x50) &&
       (berengario->estaVivo)) ||
      (berengario->aDondeVa == POS_ABAD)) {
    // si va al scriptorium a por el libro o va a avisar al abad, indica el
    // posible cambio de cámara
  ...
  if (((momentoDia == SEXTA) && (abad->aDondeHaLlegado >= 2)) ||
      (abad->estado == 0x15) || (abad->guillermoHaCogidoElPergamino) ||
      (abad->estado == 0x0b)) {
    // si en sexta va a algún lugar interesante o si va a dejar el pergamino a
    // su celda o si berengario le ha dicho que bernardo tiene el pergamino o si
    // está en estado de echar a guillermo, indica el posible cambio de cámara
  ...
  if ((malaquias->aDondeVa == POS_ABAD) ||
      ((momentoDia == VISPERAS) && (malaquias->estado < 0x06))) {
    // si va a avisar al abad o es vísperas y no ha llegado a la cocina, indica
    // el posible cambio de cámara
  ...
  // actualiza los bonus dependiendo de si guillermo y adso tienen los objetos
  // que dan bonus
  bonus |= (guillermo->objetos & (GUANTES | LLAVE1 | LLAVE2)) |
           (adso->objetos & LLAVE3);
...
void Logica::compruebaBonusYCambiosDeCamara() {
  ...
  // si estamos en la conversación con jorge sobre el libro, la cámara sigue a
  // jorge
  if (((guillermo->objetos & GUANTES) == GUANTES) &&
      ((jorge->estado == 0x0d) || (jorge->estado == 0x0e) ||
       (jorge->estado == 0x0f))) {
    cntMovimiento = 0x32;
    ...
    if ((losControles->estaSiendoPulsado(P1_UP)) ||
        (losControles->estaSiendoPulsado(P1_LEFT)) ||
        (losControles->estaSiendoPulsado(P1_RIGHT))) {
      teclaPulsada = true;
    }
  }

  // si no se pulsa ninguna tecla y el contador llega al umbral, comprueba los
  // cambios de cámara
  if (!teclaPulsada) {
    cntMovimiento++;
...
void Logica::compruebaCogerObjetos() {
  ...
  for (int i = 0; i < Juego::numPersonajes; i++) {
    ...
    // si el personaje está cogiendo o dejando un objeto, pasa al siguiente
    // personaje
    pers->contadorObjetos--;
    if (pers->contadorObjetos != -1)
      return;
    pers->contadorObjetos++;

    // elimina de la máscara de los objetos que podemos coger los que ya tenemos
    int objetosACoger =
        (pers->mascaraObjetos ^ pers->objetos) & pers->mascaraObjetos;
    ...
    for (int j = 0; j < Juego::numObjetos; j++) {
      ...
      if ((objetosACoger & mascara) == 0)
        continue;

      // si el personaje ha cogido el objeto, pasa al siguiente personaje
      if (elJuego->objetos[j]->seHaCogidoPor(pers, mascara))
        break;
    }
  }
}
...
void Logica::dejaObjeto(Personaje *pers) {
  ...
  if (numObj != -1) {
    elJuego->objetos[numObj]->dejar(pers, 1 << (Juego::numObjetos - 1 - numObj),
                                    posXObj, posYObj, alturaObj);
  }
}
...
void Logica::compruebaCogerDejarObjetos() {
  ...
  guillermo->permisosPuertas = (guillermo->permisosPuertas) |
                               ((guillermo->objetos & LLAVE1) >> 3) |
                               (guillermo->objetos & LLAVE2);
  adso->permisosPuertas =
      (adso->permisosPuertas & 0xef) | ((adso->objetos & LLAVE3) << 3);

  // calcula los objetos que se han cambiado
  int difObjetos = guillermo->objetos ^ objetosGuillermo;

  // si ha cambiado el estado de las gafas o el pergamino, y si tenemos los 2
  // objetos, comprueba si hay que generar el número del espejo y muestra el
  // texto del pergamino
...
void Logica::compruebaAbrirCerrarPuertas() {
  for (int i = 0; i < Juego::numPuertas; i++) {
    ...
    if (elMotorGrafico->actualizaCoordCamara(puerta, posXPant, posYPant,
                                             sprPosY) != -1) {
...
void Logica::ejecutaAccionesMomentoDia() {
  ...
  if ((elMotorGrafico->personaje->contadorAnimacion & 0x01) != 0)
    return;
  ...
  // si ha cambiado el momento del día, ejecuta unas acciones dependiendo del
  // momento del día
  avanzarMomentoDia = false;
...
void Logica::compruebaFinLampara() {
  ...
  if (elMotorGrafico->pantallaIluminada)
    return;
  ...
  if ((tiempoUsoLampara & 0xff) != 0)
    return;

  // si no se ha procesado todavía el último cambio en el estado de la lámpara,
  // sale
  if (cambioEstadoLampara != 0)
    return;

  if (((tiempoUsoLampara >> 8) & 0xff) == 3) {
    // si el tiempo de uso de la lámpara llega a 0x300, indica que se está
    // agotando la lámpara
    cambioEstadoLampara = 1;
  } else if (((tiempoUsoLampara >> 8) & 0xff) == 6) {
    // si el tiempo de uso de la lámpara llega a 0x600, indica que se ha agotado
    // la lámpara
...
void Logica::compruebaFinMomentoDia() {
  ...
  // NOTA: usar esto con mucho cuidado ya que puede romper la lógica normal del
  // juego al no producirse algunos eventos
  if (losControles->seHaPulsado(KEYBOARD_INTRO) && elJuego->modoInformacion) {
    elMarcador->avanzaMomentoDia();
  }

  // si esta etapa del día tiene una duración programada, comprueba si ha
  // terminado
  if (duracionMomentoDia != 0) {
    duracionMomentoDia--;
...
// calcula el porcentaje de misión completada. Si se ha completado el juego,
// muestra el final
...
void Logica::reiniciaContadoresLampara() {
  ...
  if (((malaquias->objetos & LAMPARA) == 0) && (tiempoUsoLampara == 0))
    return;
...
// si el espejo está cerrado, actualiza los sprites de los reflejos de adso y
// guillermo
void Logica::realizaReflejoEspejo() {
  if (espejoCerrado) {
    if (!reflejaPersonaje(guillermo,
                          elJuego->sprites[Juego::spritesReflejos])) {
...
// comprueba si un personaje está enfrente del espejo, y si es así, rellena el
// sprite con su reflejo
bool Logica::reflejaPersonaje(Personaje *pers, Sprite *spr) {
  // si no se está en la habitación del espejo, sale
  if (elMotorGrafico->rejilla->minPosX != 0x1c)
    return false;
  if (elMotorGrafico->rejilla->minPosY != 0x5c)
    return false;
  if (elMotorGrafico->obtenerAlturaBasePlanta(pers->altura) != 0x16)
    return false;

  // si el personaje no está a una altura donde puede reflejarse en el espejo,
  // sale
  if ((pers->altura - elMotorGrafico->obtenerAlturaBasePlanta(pers->altura)) >=
      8)
    return false;
...
// comprueba si se está delante del espejo y si se ha pulsado la Q y la R en
...
int Logica::pulsadoQR() {
  ...
  if (!losControles->estaSiendoPulsado(KEYBOARD_Q) ||
      !losControles->estaSiendoPulsado(KEYBOARD_R)) {
    return 0;
  }
...
// comprueba si se ha pulsado QR en la habitación del espejo y actúa en
// consecuencia
void Logica::compruebaAbreEspejo() {
  ...
  if (estadoQR == numeroRomano) {
    // modifica los datos de altura de la habitación del espejo para que
    // guillermo puede atravesarlo
    roms[despDatosAlturaEspejo] = 0xff;

    // cambia los datos de un bloque de la habitación del espejo para que el
    // espejo esté abierto
    ...
    // cambia los datos de un bloque de la habitación del espejo para que se
    // abra una trampa y se caiga guillermo
...
// si no se había generado el número romano para el enigma de la habitación del
// espejo, lo genera
void Logica::generaNumeroRomano() {
  static const char *tablaNumerosRomanos[3] = {"IXX", "XIX", "XXI"};
  ...
  if (numeroRomano == 0) {
    ...
    for (int i = 0; i < 3; i++) {
      gestorFrases->frasePergamino[37 + i] =
          tablaNumerosRomanos[numeroRomano - 1][i];
    }
  }

  // escribe en el marcador la frase: SECRETUM FINISH AFRICAE, MANUS SUPRA XXX
  // AGE PRIMUM ET SEPTIMUM DE QUATOR
...
void Logica::despHabitacionEspejo() {
  ...
  // recorre los datos que forman la habitación del espejo buscando el bloque
  // del espejo
...
void Logica::iniciaHabitacionEspejo() {
  // inicialmente, el espejo está cerrado y no se ha generado el número romano
  // para el enigma del espejo
  espejoCerrado = true;
  numeroRomano = 0;
...
void Logica::iniciaPersonajes() {
  ...
  guillermo->mascaraObjetos =
      LIBRO | GUANTES | GAFAS | PERGAMINO | LLAVE1 | LLAVE2;
...
void Logica::iniciaPuertas() {
  ...
  // primera puerta que cierra el paso a la parte izquierda de la planta baja de
  // la abadía
  ...
  // segunda puerta que cierra el paso a la parte izquierda de la planta baja de
  // la abadía
