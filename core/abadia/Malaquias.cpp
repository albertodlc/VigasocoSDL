...
PosicionJuego Malaquias::posicionesPredef[9] = {
    ...
    PosicionJuego(ABAJO, 0x37, 0x38,
                  0x0f), // posición en la mesa que está en la entrada del
                         // pasillo para poder subir a la biblioteca
    PosicionJuego(IZQUIERDA, 0x3a, 0x34,
                  0x0f), // posición para cerrar el paso al pasillo que lleva a
                         // la biblioteca
    PosicionJuego(DERECHA, 0x5d, 0x77,
                  0x00), // posición para cerrar las 2 puertas del ala izquierda
                         // de la abadía
    PosicionJuego(DERECHA, 0x58, 0x2a,
                  0x00), // posición en frente de la mesa de la cocina de
                         // delante del pasadizo
    PosicionJuego(
        ABAJO, 0x35, 0x37,
        0x13), // posición donde deja la llave en la mesa que está en la entrada
               // del pasillo para poder subir a la biblioteca
...
//		0x03 -> estado después de advertir a guillermo que abandone el
//scriptorium 		0x04 -> una vez que guillermo ha bajado del scriptorium, va a
//cerrar las puertas del ala izquierda 		0x05 -> una vez cerradas las puertas, se
//dirige a la cocina 		0x06 -> va a la iglesia (en vísperas) 		0x07 -> después de
//que haya llegado a su sitio en la iglesia 		0x08 -> si se va a su celda en
//completas/noche 		0x09 -> va a la iglesia (en prima) 		0x0a -> estado en el que va
//a su mesa de trabajo 		0x0b -> estado en el que empieza a morirse 		0x0c -> va a
//buscar al abad para que eche a guillermo
void Malaquias::piensa() {
  ...
  if (laLogica->momentoDia == VISPERAS) {
    ...
    if (estado < 0x04) {
      ...
      // si está yéndose del scriptorium y pasa cerca de guillermo, le advierte
      // que debe marcharse
      if (estado == 0x02) {
        // si está cerca de guillermo, le dice que abandone el scriptorium, pasa
        // al estado 3 e inicia un contador de tiempo
      ...
      // si le ha advertido a guillermo para que se marche y no le hace caso,
      // advierte al abad
      if (estado == 0x03) {
        contadorEnScriptorium++;

        // si no ha hecho caso y no ha abandonado el scriptorium, va a buscar al
        // abad
    ...
    // si guillermo ya se ha marchado del scriptorium, va a cerrar las puertas
    // del ala izquierda de la abadía
    if (estado == 0x04) {
      aDondeVa = 4;

      // si ha llegado a las puertas y no han salido berengario o bernardo, los
      // espera
      if (aDondeHaLlegado == 4) {
        if ((laLogica->berengario->estaVivo &&
             (laLogica->berengario->posX < 0x62)) ||
            (laLogica->bernardo->estaEnLaAbadia &&
             (laLogica->bernardo->posX < 0x62))) {
          elBuscadorDeRutas->seBuscaRuta = false;
        } else {
    ...
    // después de cerrar las puertas, se va a la mesa de la cocina delante del
    // pasadizo
    if (estado == 0x05) {
      aDondeVa = 5;
      laLogica->mascaraPuertas = 0xdf;

      // si guillermo se ha quedado en el ala izquierda de la abadía, advierte
      // al abad
    ...
    if (estado == 0x07) {
      // si es el quinto día y está en la iglesia, pasa al estado en el que
      // empieza a morirse
      if (laLogica->dia == 5) {
        if ((elMotorGrafico->numPantalla == 0x22) ||
            (elMotorGrafico->numPantalla == 0x22)) {
          ...
          // pone en el marcador la frase ERA VERDAD, TENIA EL PODER DE MIL
          // ESCORPIONES
  ...
  // si malaquías ha llegado a su puesto en el scriptorium, deja la llave del
  // pasadizo (si la tiene)
      ...
      if (aDondeVa == 3) {
        ...
        if ((estado2 & 0x80) == 0) {
          ...
          if (laLogica->guillermo->posY < 0x38) {
            ...
            // pone en el marcador la frase LO SIENTO, VENERABLE HERMANO, NO
            // PODEIS SUBIR A LA BIBLIOTECA
            elGestorFrases->muestraFraseYa(0x33);
          }
        } else {
          // si no le ha dicho que berengario le puede enseñar el scriptorium
          if ((estado2 & 0x40) == 0) {
            // si es el segundo día y no está reproduciendo una frase, le dice
            // que berengario le puede enseñar el scriptorium
            if ((laLogica->dia == 2) && (!elGestorFrases->mostrandoFrase)) {
              estado2 |= 0x40;

              // pone en el marcador la frase SI LO DESEAIS, BERENGARIO OS
              // MOSTRARA EL SCRIPTORIUM
  ...
  if (laLogica->momentoDia == TERCIA) {
    ...
    if ((estado == 0x09) && (laLogica->dia == 5)) {
      ...
      // si ha llegado a la celda de severino y éste está en la celda, mata a
      // severino y pasa al estado 0x0a
      if ((aDondeHaLlegado == 8) &&
          (laLogica->severino->aDondeHaLlegado == 2)) {
        laLogica->severino->estaVivo = false;
        laLogica->severino->posX = laLogica->severino->posY =
            laLogica->severino->altura = 0;
        estado = 0x0a;
      }
