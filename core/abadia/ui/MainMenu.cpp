...
MainMenu::MainMenu(int language,
            Abadia::Pergamino* pergamino,
            Abadia::MotorGrafico* gameEngine,
            Abadia::Logica* gameLogic,        
            Abadia::Controles* inputController,
            Abadia::Marcador* marcador,
            CPC6128* cpc6128,
            TimingHandler* timer)
    : 
      _language(language),
      _pergamino(pergamino),
...
bool MainMenu::process(int seleccionado) {
    ...
    while(!salir) {	
        ...
        if (_inputController->estaSiendoPulsado(KEYBOARD_0) ||
            pulsado==0 )
        {
            ...
            pulsado==1 )
        {
            ...
            _inputController->estaSiendoPulsado(KEYBOARD_2) ||
            pulsado==2 
        )
        {
            // menuGrabar();
            ...
            pulsado==4 )
        {
            ...
            _inputController->estaSiendoPulsado(KEYBOARD_5) ||
            pulsado==5 
        )
        {
            // menuTutorial();
            ...
            _inputController->estaSiendoPulsado(KEYBOARD_6) ||
            pulsado==6 
        )
        {
        ...
        if (_inputController->estaSiendoPulsado(KEYBOARD_7) ||
            pulsado==7 )
        {
            // mute=!mute;
        ...
        if (
            _inputController->estaSiendoPulsado(KEYBOARD_8) ||
            pulsado==8 
        )
        {
            salir=true;
...
bool MainMenu::processLanguageMenu(int seleccionado){
    ...
    while(!salir) {	
        ...
        if (_inputController->estaSiendoPulsado(KEYBOARD_1) ||
            pulsado==1 )
        {
            _language=1;
            salir=true;
        }
        if (_inputController->estaSiendoPulsado(KEYBOARD_2) ||
            pulsado==2 )
        {
            _language=2;
            salir=true;
        }
        if (_inputController->estaSiendoPulsado(KEYBOARD_3) ||
            pulsado==3 )
        {
            _language=3;
            salir=true;
        }
        if (_inputController->estaSiendoPulsado(KEYBOARD_4) ||
            pulsado==4 )
        {
            _language=4;
            salir=true;
        }
        if (_inputController->estaSiendoPulsado(KEYBOARD_5) ||
            pulsado==5 )
        {
            _language=5;
            salir=true;
        }
        if (_inputController->estaSiendoPulsado(KEYBOARD_6) ||
            pulsado==6 )
        {
            _language=6;
            salir=true;
...
void MainMenu::renderMenu(int selected, bool enableEffect){
	...
	for(int x = enableEffect ? 8 : 88; x < 88; x += 10)
	{
		...
		for (int i=0;i<9;i++)
		{
			_marcador->imprimeFrase(textos[_language][i], x, 
				16+(i*16),4, 0);
		}
		_timer->sleep(50);
	}
	...
	// de letra cambiado 
	_marcador->imprimeFrase(textos[_language][selected], 88, 16+(selected*16), 0, 4);
}
...
void MainMenu::renderLanguageMenu(int selected, bool enableEffect){
	...
	// pinta la opción seleccionado con el color de fondo y el color
	// de letra cambiado 
	_marcador->imprimeFrase(textos[_language][selected], 88, 32+(selected*16), 0, 4);
...
void MainMenu::renderRecordingMenu(int seleccionado, bool enableEffect)
{
	...
	for(int x=enableEffect?8:88;x<88;x+=10)
	{
		...
		for (int i=0;i<8;i++)
		{
			_marcador->imprimeFrase(textos[_language][i], 
				x, 32+(i*16),4, 0);
		}
		_timer->sleep(50);
	}
...
bool MainMenu::processIntro()
{
	...
	while (espera){
		_inputController->actualizaEstado();
		_timer->sleep(1);
		espera = _inputController->estaSiendoPulsado(
				P1_BUTTON1);
	}
...
void MainMenu::renderKeyboardMenu(int seleccionado)
{
	...
	// pinta la opción seleccionado con el color de fondo y el color
	// de letra cambiado 
	_marcador->imprimeFrase(textos[_language][seleccionado], 8, 16+(seleccionado*16), 0, 4);
...
void MainMenu::renderCameraMenu(int seleccionado)
{
	...
	// pinta la opción seleccionado con el color de fondo y el color
	// de letra cambiado 
	_marcador->imprimeFrase(textos[_language][seleccionado], 1, 16+(seleccionado*16), 0, 4);
...
void MainMenu::renderTutorialMenu(int seleccionado, bool enableEffect)
{
	...
	// pinta la opción seleccionado con el color de fondo y el color
	// de letra cambiado 
	_marcador->imprimeFrase(textos[_language][seleccionado], 88, 32+(seleccionado*16), 0, 4);
