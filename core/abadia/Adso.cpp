...
// Hay 2 grandes grupos de entradas en la tabla. Las primeras 4 entradas dan más
// prioridad a los movimientos a la derecha y las 4 últimas a los movimientos a
// la izquierda. Dentro de cada grupo de entradas, las 2 primeras entradas dan
// más prioridad a los movimientos hacia abajo, y las otras 2 entradas dan más
// prioridad a los movimientos hacia arriba.
/////////////////////////////////////////////////////////////////////////////

int Adso::oriMovimiento[8][4] = {
    {3, 0, 2, 1}, {0, 3, 1, 2}, {1, 0, 2, 3}, {0, 1, 3, 2},

    {3, 2, 0, 1}, {2, 3, 1, 0}, {1, 2, 0, 3}, {2, 1, 3, 0}};

/////////////////////////////////////////////////////////////////////////////
// tabla de desplzamientos dentro del buffer de alturas según la orientación
/////////////////////////////////////////////////////////////////////////////

int Adso::despBufferSegunOri[4][2] = {{+1, 0}, {0, -1}, {-1, 0}, {0, +1}};
...
// VGA
Personaje::DatosFotograma Adso::tablaAnimacion[8] = {
    {58408, 0x05, 0x20}, {57768, 0x05, 0x20}, {58408, 0x05, 0x20},
    {59048, 0x05, 0x1f}, {59668, 0x04, 0x1e}, {60148, 0x04, 0x1e},
    {59668, 0x04, 0x1e}, {60628, 0x04, 0x1e}};
...
void Adso::run() {
  ...
  // modifica la tabla de conexiones de las habitaciones dependiendo de las
  // puertas a las que se tenga acceso
  ...
  if (aDondeVa != POS_GUILLERMO) {
    // si se han terminado los comandos de movimiento, genera los comandos para
    // ir a donde se quiere
    ...
    if (laLogica->numPersonajeCamara >= 2)
      return;
    ...
    if (rejilla->estaEnRejillaCentral(this, posXRejilla, posYRejilla)) {
      ...
      if (losControles->estaSiendoPulsado(P1_UP)) {
        ...
        rejilla->obtenerAlturaPosicionesAvance2(guillermo, difAltura1,
                                                difAltura2, avanceX, avanceY);

        // comprueba si adso está en alguna de las posiciones a las que va a
        // moverse guillermo
        int aux =
            rejilla->bufCalculoAvance[0][1] | rejilla->bufCalculoAvance[0][2] |
            rejilla->bufCalculoAvance[1][1] | rejilla->bufCalculoAvance[1][2];
      ...
      // si se pulsó cursor abajo, trata de avanzar en la orientación de
      // guillermo
      ...
      // limpia las posiciones que ocupan adso y guillermo en el buffer de
      // alturas
      ...
      bool noHayError =
          rejilla->ajustaAPosRejilla(posX, posY, posXDest, posYDest);
      noHayError |= rejilla->ajustaAPosRejilla(guillermo->posX, guillermo->posY,
                                               elBuscadorDeRutas->posXIni,
                                               elBuscadorDeRutas->posYIni);
      assert(noHayError);
      ...
      // restaura las posiciones que ocupan adso y guillermo en el buffer de
      // alturas
      ...
      // si el número de iteraciones para llegar a guillermo es grande, avanza
      // hacia guillermo
...
//		0x04 -> estado dentro de su celda, esperando contestación de si
//duerme o no 		0x05 -> estado que se alcanza una vez que guillermo le dice que no
//se duerme 		0x06 -> estado fuera de la celda por la noche o en completas para
//dirigirse a su celda 		0x07 -> estado en sexta para ir al refectorio
void Adso::piensa() {
  int numFrase = 0;
  Personaje *guillermo = laLogica->guillermo;
  ...
  // si se está acabando la noche, pone en el marcador la frase: PRONTO
  // AMANECERA, MAESTRO
  ...
  if (!laLogica->haFracasado) {
    ...
    if (laLogica->cntTiempoAOscuras >= 1) {
      ...
      // si se termina el contador, muestra la frase JAMAS CONSEGUIREMOS SALIR
      // DE AQUI y termina la partida
      if (laLogica->cntTiempoAOscuras == 1) {
      ...
      if (altura >= 0x18) {
        ...
        // si adso no tiene la lámpara, muestra la frase: DEBEMOS ENCONTRAR UNA
        // LAMPARA, MAESTRO
        ...
        // si adso no está en la biblioteca, apaga la lámpara y reinicia el
        // contador del tiempo que pueden ir a oscuras
        laLogica->usandoLampara = false;
        laLogica->cntTiempoAOscuras = 0;
      }
    }
  ...
  case NOCHE:
    ...
    if (estado == 4) {
      // si está en la pantalla de fuera de la celda, indica que hay que avanzar
      // el momento del día
      ...
      if (estado == 6) {
        ...
        if (estaCerca(guillermo) && (elMotorGrafico->numPantalla == 0x3e)) {
          // reinicia el contador para dormir y pone la frase ¿DORMIMOS?,
          // MAESTRO
          cntParaDormir = 0;
          estado = 4;
...
void Adso::grabaComandosAvance(int nuevaOri) {
  ...
  elMotorGrafico->rejilla->obtenerAlturaPosicionesAvance(
      this, difAltura1, difAltura2, avanceX, avanceY);
...
void Adso::pruebaMover(int numEntrada) {
  ...
  int alturaBase =
      elMotorGrafico->rejilla->bufAlturas[posYRejilla][posXRejilla] & 0x0f;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (elBuscadorDeRutas->esPosicionDestino(posX, posY, altura, alturaBase,
                                             false)) {
      elMotorGrafico->rejilla->bufAlturas[posY][posX] &= 0x7f;
