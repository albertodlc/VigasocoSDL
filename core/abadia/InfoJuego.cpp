...
int InfoJuego::zonaVisiblePlanta[3][4] = {{0x01, 0x0c, 0x01, 0x0a},
                                          {0x01, 0x05, 0x02, 0x06},
                                          {0x01, 0x05, 0x02, 0x06}};

int InfoJuego::alturaBasePlanta[3] = {0x00, 0x0d, 0x18};

/////////////////////////////////////////////////////////////////////////////
// métodos para mostrar información interna del juego
/////////////////////////////////////////////////////////////////////////////

// inicia los datos necesarios para poder mostrar la información sobre el
// comportamiento del juego más tarde
void InfoJuego::inicia() {
  // inicia la paleta (los colores colores 0-3 no pueden usarse ya que los usa
  // el juego)
  ...
  for (int i = 0; i < 16; i++) {
    ...
    paleta->setColor(200 + 16 + i, 0x80 + paso * i, 0x80 + paso * i,
                     0x80 + paso * i);
  }
...
void InfoJuego::muestraInfo() {
  /* las teclas 1 a 7 ahora se usan para hacer trampas y cambiar la camara para
     seguir a otros personajes if (losControles->seHaPulsado(KEYBOARD_1))
     numPersonaje = (numPersonaje + 1) % (Juego::numPersonajes + 1); if
     (losControles->seHaPulsado(KEYBOARD_2)) numObjeto = (numObjeto + 1) %
     (Juego::numObjetos + 1); if (losControles->seHaPulsado(KEYBOARD_3))
     numPuerta = (numPuerta + 1) % (Juego::numPuertas + 1); if
     (losControles->seHaPulsado(KEYBOARD_4)) mostrarLogica = !mostrarLogica; if
     (losControles->seHaPulsado(KEYBOARD_5)) mostrarRejilla = !mostrarRejilla;
          if (losControles->seHaPulsado(KEYBOARD_6)) mostrarMapaPlantaActual =
     !mostrarMapaPlantaActual; if (losControles->seHaPulsado(KEYBOARD_7))
     mostrarMapaRestoPlantas = !mostrarMapaRestoPlantas;
  */
  /*
          // como con el plugin actual de SDLVideo no se muestran los textos
          // de infojuego
          // solo remapeo teclas para mostrar mapas
          if (losControles->seHaPulsado(KEYBOARD_8)) mostrarRejilla =
     !mostrarRejilla; if (losControles->seHaPulsado(KEYBOARD_9))
     mostrarMapaPlantaActual = !mostrarMapaPlantaActual; if
     (losControles->seHaPulsado(KEYBOARD_0)) mostrarMapaRestoPlantas =
     !mostrarMapaRestoPlantas;
  */
  ...
  int planta =
      elMotorGrafico->obtenerPlanta(elMotorGrafico->obtenerAlturaBasePlanta(
...
void InfoJuego::dibujaPosicionRejilla(int x, int y, int valor) {
  ...
  if (valor < 0x10) {
    ...
    static int gfxPosPersonaje[4][4] = {
        {15, 0, 0, 15}, {0, 15, 15, 0}, {0, 15, 15, 0}, {15, 0, 0, 15}};

    for (int j = 0; j < 4; j++) {
      for (int i = 0; i < 4; i++) {
        dibujaPixelCuadrado(
            4 * x + i, 4 * y + j,
            (gfxPosPersonaje[j][i] == 0) ? color : gfxPosPersonaje[j][i]);
      }
    }
  }
}
...
void InfoJuego::generaAlturasPlanta() {
  // crea un personaje ficticio para generar la altura de todas las pantallas de
  // la planta
  Sprite spr;
  Guillermo pers(&spr);

  // crea un objeto rejilla para obtener la altura de la pantalla en la que se
    ...
    for (int j = minY; j <= maxY; j++) {
      for (int i = minX; i <= maxX; i++) {
        ...
        if ((mapa[16 * j + i] != 0) ||
          ...
          UINT8 *alturaPantalla =
              &alturas[(longX * (j - minY) + i - minX) * 16 * 16];

          // recorre las 16x16 posiciones centrales de la rejilla y guarda su
          // altura
          for (int y = 0; y < 16; y++) {
            for (int x = 0; x < 16; x++) {
              alturaPantalla[16 * y + x] =
                  0x10 + ((rejilla->bufAlturas[y + 4][x + 4]) & 0x0f);
            }
          }
        }
      }
  ...
  for (int j = minY; j <= maxY; j++) {
    for (int i = minX; i <= maxX; i++) {
      ...
      if ((mapa[16 * j + i] != 0) ||
        ...
        UINT8 *alturaPantalla =
            &alturas[(longX * (j - minY) + i - minX) * 16 * 16];
        dibujaAlturaPosicionPlanta(posX, 0, i - minX, j - minY, zoom,
                                   alturaPantalla);
      }
    }
  }
...
// dibuja la altura de una pantalla de la planta con un determinado factor de
// zoom
void InfoJuego::dibujaAlturaPosicionPlanta(int posX, int posY, int i, int j,
                                           int zoom, UINT8 *alturas) {
  for (int y = 0; y < 16; y++) {
    for (int x = 0; x < 16; x++) {
      for (int zoomY = 0; zoomY < zoom; zoomY++) {
        for (int zoomX = 0; zoomX < zoom; zoomX++) {
          dibujaPixelCuadrado(posX + (16 * i + x) * zoom + zoomX,
                              posY + (16 * j + y) * zoom + zoomY,
                              alturas[16 * y + x]);
        }
      }
    }
...
void InfoJuego::muestraPosicionMapaAlturas(int numPlanta) {
  ...
  for (int i = 0; i < Juego::numPersonajes; i++) {
    ...
    if ((pers->posX == 0) && (pers->posY == 0) && (pers->altura == 0))
      continue;

    // si el personaje no está en esta planta, pasa al siguiente
    if (elMotorGrafico->obtenerPlanta(
            elMotorGrafico->obtenerAlturaBasePlanta(pers->altura)) != numPlanta)
      continue;
...
void InfoJuego::muestraPosicionMapa(int posX, int numPlanta) {
  ...
  for (int i = 0; i < Juego::numPersonajes; i++) {
    ...
    // calcula el desplazamiento dentro del mapa
    int despX = (((pers->posX & 0xf0) >> 4) - minX) * zoom +
                (pers->posX & 0x0f) * zoom / 16;
    int despY = (((pers->posY & 0xf0) >> 4) - minY) * zoom +
                (pers->posY & 0x0f) * zoom / 16;

    bool pixelValido = (((pers->posX & 0xf0) >> 4) >= minX) &&
                       (((pers->posX & 0xf0) >> 4) <= maxX) &&
                       (((pers->posY & 0xf0) >> 4) >= minY) &&
                       (((pers->posY & 0xf0) >> 4) <= maxY);
    // assert(pixelValido);
...
void InfoJuego::muestraInfoLogica(int x, int y) {
  ...
  strBuf << "Variables de logica" << std::endl
         ...
         << "durMomentoDia = " << muestra(laLogica->duracionMomentoDia)
         << std::endl
         << "oldMomentoDia = " << muestra(laLogica->oldMomentoDia) << std::endl
         << "avanzMomentoDia = " << muestra(laLogica->avanzarMomentoDia)
         << std::endl
         << "obsequium = " << muestra(laLogica->obsequium) << std::endl
         << "haFracasado = " << muestra(laLogica->haFracasado) << std::endl
         << "investigacionCompleta = "
         << muestra(laLogica->investigacionCompleta) << std::endl
         << "bonus = " << muestra(laLogica->bonus) << std::endl

         << "mascaraPuertas = " << muestra(laLogica->mascaraPuertas)
         << std::endl

         << "espejoCerrado = " << muestra(laLogica->espejoCerrado) << std::endl
         << "numeroRomano = " << muestra(laLogica->numeroRomano) << std::endl

         << "seAcabaLaNoche = " << muestra(laLogica->seAcabaLaNoche)
         << std::endl
         << "haAmanecido = " << muestra(laLogica->haAmanecido) << std::endl
         << "usandoLampara = " << muestra(laLogica->usandoLampara) << std::endl
         << "tiempoUsoLampara = " << muestra(laLogica->tiempoUsoLampara)
         << std::endl
         << "cambioEstadoLampara = " << muestra(laLogica->cambioEstadoLampara)
         << std::endl
         << "cntTiempoAOscuras = " << muestra(laLogica->cntTiempoAOscuras)
         << std::endl

         << "cntLeeLibroSinGuantes = "
         << muestra(laLogica->cntLeeLibroSinGuantes) << std::endl
         << "pergaminoGuardado = " << muestra(laLogica->pergaminoGuardado)
         << std::endl

         << "hayMovimiento = " << muestra(laLogica->hayMovimiento) << std::endl
         << "cntMovimiento = " << muestra(laLogica->cntMovimiento) << std::endl

         << "numPersonajeCamara = " << muestra(laLogica->numPersonajeCamara)
         << std::endl
         << "opcionPersonajeCamara = "
         << muestra(laLogica->opcionPersonajeCamara) << std::endl;
...
std::string InfoJuego::muestraEntidad(EntidadJuego *entidad) {
  std::ostringstream strBuf;
  strBuf << "pos = (" + muestra(entidad->posX) + ", " + muestra(entidad->posY) +
                ", " + muestra(entidad->altura) + ")"
         << std::endl
...
std::string InfoJuego::muestraPersonaje(int i, Personaje *pers) {
  ...
  if (i != 0) {
    ...
    strBuf << "numBitAcciones = " << muestra(pIA->numBitAcciones) << std::endl
           << "pensarNuevoMov = " << muestra(pIA->pensarNuevoMovimiento)
           << std::endl
...
std::string InfoJuego::muestraPuerta(Puerta *puerta) {
  ...
  strBuf << muestraEntidad(puerta)
         ...
         << "hayQueRedibujar = " << muestra(puerta->hayQueRedibujar)
         << std::endl;
  return strBuf.str();
}
...
std::string InfoJuego::muestraObjeto(Objeto *obj) {
  ...
  if (!obj->seHaCogido) {
    ...
    strBuf << "pos = (" + muestra(obj->personaje->posX) + ", " +
                  muestra(obj->personaje->posY) + ", " +
                  muestra(obj->personaje->altura) + ")"
           << std::endl
           << "orientacion = " << muestra(obj->personaje->orientacion)
           << std::endl
           << "seEstaCogiendo = " << muestra(obj->seEstaCogiendo) << std::endl
           << "seHaCogido = " << muestra(obj->seHaCogido) << std::endl;
  }
...
std::string InfoJuego::muestraSprite(Sprite *spr) {
  ...
  strBuf << "esVisible = " << muestra(spr->esVisible) << std::endl
         ...
         << "posPant = (" << muestra(spr->posXPant) << ", "
         << muestra(spr->posYPant) << ")" << std::endl
         << "oldPosPant = (" << muestra(spr->oldPosXPant) << ", "
         << muestra(spr->oldPosYPant) << ")" << std::endl
         << "dim = (" << muestra(spr->ancho) << ", " << muestra(spr->alto)
         << ")" << std::endl
         << "oldDim = (" << muestra(spr->oldAncho) << ", "
         << muestra(spr->oldAlto) << ")" << std::endl
         << "posTile = (" << muestra(spr->posXTile) << ", "
         << muestra(spr->posYTile) << ")" << std::endl
         << "dimFinal = (" << muestra(spr->anchoFinal) << ", "
         << muestra(spr->altoFinal) << ")" << std::endl
         << "posLocal = (" << muestra(spr->posXLocal) << ", "
         << muestra(spr->posYLocal) << ")" << std::endl
...
void InfoJuego::dibujaPixelCuadradoZoom(int posX, int posY, int i, int j,
                                        int zoom, int color) {
  for (int zoomY = 0; zoomY < zoom; zoomY++) {
    for (int zoomX = 0; zoomX < zoom; zoomX++) {
      dibujaPixelCuadrado(posX + i * zoom + zoomX, posY + j * zoom + zoomY,
                          color);
    }
  }
}
