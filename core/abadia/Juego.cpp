...
const char *Juego::savefile[7] = {
    "mc0:ABADIA/abadia0.save", "mc0:ABADIA/abadia1.save",
    "mc0:ABADIA/abadia2.save", "mc0:ABADIA/abadia3.save",
    "mc0:ABADIA/abadia4.save", "mc0:ABADIA/abadia5.save",
    "mc0:ABADIA/abadia6.save"};
#else
#ifdef __native_client__
const char *Juego::savefile[7] = {"/save/abadia0.save", "/save/abadia1.save",
                                  "/save/abadia2.save", "/save/abadia3.save",
                                  "/save/abadia4.save", "/save/abadia5.save",
                                  "/save/abadia6.save"};
#else
const char *Juego::savefile[7] = {
    "abadia0.save", "abadia1.save", "abadia2.save", "abadia3.save",
    "abadia4.save", "abadia5.save", "abadia6.save"};
#endif
#endif
...
Juego::Juego(UINT8 *romData, CPC6128 *cpc) {
  ...
  // paleta = new Paleta();
  paleta = new Paleta(romData + 0x24000 -
                      1); // le pasamos los datos de la paleta VGA
  ...
  _mainMenu = new MainMenu(idioma, pergamino, motor, logica, controles,
                           marcador, cpc6128, timer);
...
Juego::~Juego() {
  ...
  delete pergamino;
  delete paleta;
  // CRITICAL: Since we used 'new', we MUST use 'delete' to avoid memory leaks
  delete _mainMenu;
 ...
 *
 * @return false al salir
 */
bool Juego::menu() { return _mainMenu->process(); }
...
void Juego::run() {
  ...
  _mainMenu->setTimer(timer);
  _mainMenu->setInputController(controles);
  ...
  despues_de_cargar_o_iniciar:
    ...
    while (true) {
      ...
      // obtiene el contador de la animación de guillermo para saber si se
      // generan caminos en esta iteración
      elBuscadorDeRutas->contadorAnimGuillermo =
          laLogica->guillermo->contadorAnimacion;
      ...
      // comprueba si hay que avanzar la parte del momento del día en el
      // marcador
      marcador->realizaScrollMomentoDia();

      // comprueba si hay que ejecutar las acciones programadas según el momento
      // del día
      logica->ejecutaAccionesMomentoDia();

      // comprueba si hay opciones de que la cámara siga a otro personaje y
      // calcula los bonus obtenidos
...
// TODO: Mover a clase donde se pueda reutilizar
...
void Juego::generaGraficosFlipeadosVGA() {
  ...
  dest += size;
  size = 57768 -
         57240; // ok, coincide con los 0x084 que se pasaba para CPC * 4 ya que
                // en VGA cada pixel es un byte , y no 4 pixel en un byte
  ...
  // genera los gráficos de las caras de los monjes flipeados respecto a x
  dest += size;
  dest += 2900; // TODO: cambio temporal, para que la distancia entre los
                // graficos y su homologo flipeados sea siempre 120305
  // a ver si hay suerte, y en estos 2900 bytes lo que estan son los graficos de
  // las puertas que son los que nos faltan !!! -> pues no, estan justo detras
  // de las caras
...
void Juego::flipeaGraficos(UINT8 *tablaFlip, UINT8 *src, UINT8 *dest, int ancho,
                           int bytes) {
...
void Juego::actualizaLuz() {
  ...
  // actualiza las características del sprite de la luz según la posición del
  // personaje
...
void Juego::cambioCPC_VGA() {
  ...
  if (GraficosCPC) {
    memcpy(&roms[0x24000 - 1 - 0x4000],
           &roms[0x24000 - 1 - 0x4000 + (174065 + 21600)], 174065);

    GraficosCPC = false;
  } else {
    memcpy(&roms[0x24000 - 1 - 0x4000],
           &roms[0x24000 - 1 - 0x4000 + (174065 + 21600) * 2], 174065);
...
// ! métodos para mostrar las pantallas de distintas situaciones del juego

/**
 * @brief Muestra la imagen de presentación del juego
 *
 */
...
// muestra la parte de misión completada. Si se ha completado el juego, muestra
// el final
bool Juego::muestraPantallaFinInvestigacion() {
  ...
  // si está mostrando una frase por el marcador, espera a que se termine de
  // mostrar
...
void Juego::creaEntidadesJuego() {
  ...
  // CPC int despObjetos[8] = { 0x88f0, 0x9fb0, 0x9f80, 0xa010, 0x9fe0, 0x9fe0,
  // 0x9fe0, 0x88c0 }; VGA
  /*
  int despObjetos[8] = {
  ...
  // En Sprite::dibujaVGA ya le anyade el 0x24000 - 1 - 0x4000 para saltarse la
  // rom CPC e ir a los graficos VGA ¡¡¡ ojo !!! , entonces en
  // Marcador::dibujaObjetos hay que incluir este salto ...
  int despObjetos[8] = {
      11200, // LIBRO
      34496, // GUANTES
      34304, // GAFAS
  ...
  for (int i = primerSpriteObjetos; i < primerSpriteObjetos + numObjetos; i++) {
    ...
    // - 0x4000 ya que en el puntero con el que se trabaja se han pasado los
    // primeros 0x4000 bytes con la pantalla de presentacion CPC +34304 que es
    // donde esta el tile 228 , que es por donde anda la imagen del primer
    // objeto como los objetos son de 16*12 (¡¡¡ojo!!! no son como el resto de
    // tiles!!!) sumo 16*12* el numero de objeto que busco
    // sprites[i]->despGfx = 0x24000 - 1 - 0x4000 + 34304 + (i -
    // primerSpriteObjetos)*16*12; muy bonito , si los graficos estuviesen en el
    // mismo orden logico que los objetos en el juego

    // igual que en CPC , pero con el array despObjetos "actualizado" a las
    // posiciones VGA
