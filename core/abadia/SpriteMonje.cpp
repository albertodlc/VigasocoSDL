...
SpriteMonje::SpriteMonje() { animacionTraje = 0; }
...
que se le pasa como parámetro void SpriteMonje::dibuja(Sprite *spr, UINT8
*bufferMezclas, int lgtudClipX, int lgtudClipY, int dist1X, int dist2X, int
dist1Y, int dist2Y)
{
        ...
        // si ya ha pasado la zona de la cabeza, obtiene los gráficos de la
parte del traje if (dist2Y >= 10){ despSrc = despAnimTraje[animacionTraje] +
(dist2Y - 10)*ancho + dist2X; } else { despSrc = despGfx + dist2Y*ancho +
dist2X;
        }

        // calcula la dirección de destino de los gráficos en el buffer de
sprites int despDest = spr->despBuffer + (dist1Y*spr->anchoFinal + dist1X)*4;
...
destino if (color != 0){ *dest = color;
                ...
                // si ya se ha dibujado la cabeza, obtiene los gráficos de la
parte del traje if (dist2Y == 10){ despSrc = despAnimTraje[animacionTraje] +
dist2X;
                }
        }
...
void SpriteMonje::dibuja(Sprite *spr, UINT8 *bufferMezclas, int lgtudClipX,
  ...
  // si ya ha pasado la zona de la cabeza, obtiene los gráficos de la parte del
  // traje
  if (dist2Y >= 10) {
    despSrc =
        despAnimTraje[animacionTraje] + (dist2Y - 10) * ancho * 4 + dist2X * 4;
  } else {
  ...
  for (int lgtudY = 0; lgtudY < lgtudClipY; lgtudY++) {
    ...
    // si ya se ha dibujado la cabeza, obtiene los gráficos de la parte del
    // traje
    if (dist2Y == 10) {
