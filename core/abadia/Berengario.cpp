...
PosicionJuego Berengario::posicionesPredef[6] = {
    ...
    PosicionJuego(
        ARRIBA, 0x52, 0x67,
        0x04), // posición al pie de las escaleras para subir al scriptorium
...
void Berengario::piensa() {
  ...
  if (laLogica->momentoDia == NOCHE) {
    ...
    if (laLogica->dia == 3) {
      if (estado == 6) {
        ...
        if (aDondeHaLlegado == 3) {
          ...
          if ((objetos & LIBRO) == LIBRO) {
            // si ha llegado a la celda de severino, muere y avanza el momento
            // del día
      ...
      // si está en su celda, cambia su cara por la del encapuchado y pasa al
      // estado 6
      if (aDondeHaLlegado == 3) {
        fijaCapucha(true);
        estado = 6;
  ...
  if (laLogica->momentoDia == VISPERAS) {
    // si es el segundo día y malaquías no ha bajado del scriptorium, se queda
    // quieto protegiendo el pergamino
  ...
  if (laLogica->dia < 3) {
    ...
    if (estado == 5) {
      ...
      // si ha llegado a la posición del abad, le informa de que guillermo ha
      // cogido el pergamino
    ...
    if (aDondeHaLlegado == 2) {
      ...
      if (guillermoHaCogidoElPergamino()) {
        ...
        // si está cerca de guillermo, le dice que deje el manuscrito o
        // advertirá al abad
        if (estaCerca(laLogica->guillermo)) {
          // pone en el marcador la frase DEJAD EL MANUSCRITO DE VENACIO O
          // ADVERTIRE AL ABAD
    ...
    // si malaquías le ha dicho a guillermo que le enseñe el scriptorium y éste
    // no está en la planta baja
    if (((laLogica->malaquias->estado2 & 0x40) == 0x40) &&
        (laLogica->guillermo->altura >= 0x0d)) {
      ...
      if ((estado2 & 0x10) == 0) {
        ...
        if (estaCerca(laLogica->guillermo)) {
          if (!elGestorFrases->mostrandoFrase) {
            ...
            // pone en el marcador la frase AQUI TRABAJAN LOS MEJORES COPISTAS
            // DE OCCIDENTE
