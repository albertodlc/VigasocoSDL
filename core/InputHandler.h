...
//
//	The inputs are stored on the _inputs array. The _oldInputs array stores
//last 	4 input values. If a game doesn't use has input ports (for example, a CPC
//	game), the _inputs array will be empty and the core scans for all
//inputs.
//
//	The class has a collection of plugins that are called to update the
//_inputs 	array. All plugins should be added before calling the init method.
//
//	In a game not all inputs are needed, so the plugins only update the
//	entries >= 0. After all plugins have modified the inputs array, the
//class 	updates the game's input ports to reflect the current inputs.
//
...
public:
  ...
  UINT32 getInput(int number) const {
    return _inputPorts->at(number)->getValue();
  }
