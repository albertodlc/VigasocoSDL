...
void AbadiaDriver::createGameDataEntities() {
  ...
  // Graficos VGA
  GameDataEntity *GfxVGA =
      new GameDataEntity(GRAPHICS, "Graphics VGA + Palette");
  GfxVGA->addFile(new GameFile("GraficosVGA", 0, 174065, 0, 0));
  _gameFiles.push_back(GfxVGA);

  // Graficos CPC adaptados a modo 256 colores
  GameDataEntity *GfxCPC =
      new GameDataEntity(GRAPHICS, "Graphics CPC + Palette");
...
void AbadiaDriver::filesLoaded() {
  ...
  // pegamos al final de romsPtr los datos de GraficosVGA del "remake" a 256
  // colores

  romsPtr = NULL;
  romsPtr = new UINT8[0x24000 + ((_gameFiles[1]->getTotalSize() + 21600) * 3)];

  // TODO: La funcion filesLoaded no debe ser void y se debe revisar si se pudo
  // conseguir la memoria y devolver este resultado
  if (romsPtr == NULL)
    printf("No se pudo reservar memoria para la rom \n");

  // 0x24000 de la rom original de CPC
  // 174065 de Graficos VGA
  // 21600 para copiar los graficos VGA flipeados necesarios ... (falta ajustar
  // este valor) Y por 3, porque  tenemos primero los graficos a utilizar, y
  ...
  memcpy(&romsPtr[0x24000 - 1], _gameFiles[1]->getData(),
         _gameFiles[1]->getTotalSize());
  memcpy(&romsPtr[0x24000 - 1 + _gameFiles[1]->getTotalSize() + 21600],
         _gameFiles[1]->getData(), _gameFiles[1]->getTotalSize());
  memcpy(&romsPtr[0x24000 - 1 + (_gameFiles[1]->getTotalSize() + 21600) * 2],
         _gameFiles[2]->getData(), _gameFiles[1]->getTotalSize());
  // Los sonidos no se copian, y se cargan directamente en Audioplugin

  // En la versión 0.09 se incluyeron nuevos caracteres para soportar nuevos
  // idiomas Esos nuevos caracteres incluian '-' y '.' , a los que se le añadio
  // nuevo gráfico Pero en el código original algunos caracteres como - . > / se
  // usan para representar varios caracteres a la vez Por ejemplo, COMPLETAS no
  // cabe en el hueco del Marcador donde se muestra el día Y en la ROM está
  // grabado como COM-./> El gráfico del guión es el texto PL comprimido y
  // ajustado para que COMPLETAS entre en el marcador
  //
...
void AbadiaDriver::finishInit() {
  // crea e inicia la sección crítica para la sincronización del dibujado de
  // gráficos
...
void AbadiaDriver::audioInitialized(IAudioPlugin *ap) {
  ...
  struct {
    ...
    size_t size;
      ...
      "roms/abadia/abrir.wav",    12092,  "roms/abadia/aporrear.wav", 35000,
      "roms/abadia/campanas.wav", 192342, "roms/abadia/cerrar.wav",   15246,
      "roms/abadia/coger.wav",    7460,   "roms/abadia/dejar.wav",    7300,
      "roms/abadia/espejo.wav",   42260,  "roms/abadia/final.wav",    796898,
      "roms/abadia/fondo.wav",    528220, "roms/abadia/inicio.wav",   1478220,
      "roms/abadia/pasos.wav",    14526,  "roms/abadia/tintineo.wav", 22128};
#else
  typedef struct {
    char nombre[255];
    size_t size;
  ...
  str_sonidos sonidos[] = {
      ...
      "roms/abadia/campanas.wav", 192342, "roms/abadia/cerrar.wav",   15246,
      "roms/abadia/coger.wav",    7460,   "roms/abadia/dejar.wav",    7300,
      "roms/abadia/espejo.wav",   42260,  "roms/abadia/final.wav",    1464250,
      "roms/abadia/fondo.wav",    528220, "roms/abadia/inicio.wav",   2716180,
      "roms/abadia/pasos.wav",    14526,  "roms/abadia/tintineo.wav", 22128};
#endif
...
void AbadiaDriver::render(IDrawPlugin *dp) {
  ...
  // dibuja los datos que el juego escribe en el otro hilo en el buffer de
  // pantalla
  cs->enter();
  for (int y = 0; y < 200; y++) {
    for (int x = 0; x < 640; x++) {
