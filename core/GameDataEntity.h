...
//
//	A game data entity is a collection of files (GameFiles) with related
//data. 	Each game data entity has an associated load handler, that reorganizes
//the 	internal memory as needed.
//
//	To populate the game data entity:
//		* call preProcess in order to allocate internal memory for all
//the 		files and to do any specific load handler pre processing.
//		* for all the files in the game data entity, call loadData with
//the 		file data. The load handler will copy and process the data as needed.
...
struct GameFile {
  ...
  GameFile(std::string file, UINT32 address, UINT32 size, UINT32 crc,
           UINT32 data) {
    fileName = file;
    baseAddress = address;
    fileSize = size;
...
enum DataType { GRAPHICS, SOUND, PALETTE, MIXED };
