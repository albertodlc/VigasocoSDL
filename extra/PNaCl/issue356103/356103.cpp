...
class issue356103_Instance : public pp::Instance {
private:
  ...
  // resources are ready.
  void StartInNewThread(int32_t dummy) {
    if (pthread_create(&thread_ONE_id, NULL, &threadONE, NULL) != 0)
      printf("ERR pthread_create ONE\n");
  }

public:
  explicit issue356103_Instance(PP_Instance instance) : pp::Instance(instance) {
  ...
  virtual bool HandleInputEvent(const pp::InputEvent &event) { return true; }
...
class issue356103_Module : public pp::Module {
public:
  issue356103_Module() : pp::Module() {}
  virtual ~issue356103_Module() {}
...
Module *CreateModule() { return new issue356103_Module(); }
